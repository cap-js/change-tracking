...
async function _regenerateSQLiteTriggers(entityNames, allEntities, hierarchyMap) {
	...
	if (entityNames) {
		for (const entityName of entityNames) {
			...
			const existingTriggers = await cds.db.run(`SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE '${triggerPattern}'`);
			for (const { name } of existingTriggers) {
				await cds.db.run(`DROP TRIGGER IF EXISTS "${name}"`);
			}
		}
	} else {
		const existingTriggers = await cds.db.run(`SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE '%_ct_%'`);
		for (const { name } of existingTriggers) {
			await cds.db.run(`DROP TRIGGER IF EXISTS "${name}"`);
		}
	}
...
async function regenerateTriggers(entityNames) {
	...
	const normalizedEntityNames = entityNames ? (Array.isArray(entityNames) ? entityNames : [entityNames]) : null;
