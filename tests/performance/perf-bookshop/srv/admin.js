...
export default class AdminService extends cds.ApplicationService {
    init() {
        ...
        this.on('updateChildren', async (req) => {
            const book = await SELECT.one.from(req.subject).columns((book) => 
                book`.*`,
                book.children('*')
            )
            const children = [];
            for (const child of book.children) {
                children.push(UPDATE.entity(req.target.isDraft ? srvChapters.drafts : Chapters).where({ID: child.ID}).set({
