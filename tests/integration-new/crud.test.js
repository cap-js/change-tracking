...
describe('CRUD Operations', () => {
    ...
    describe('Create Tracking', () => {
        ...
        it('should track multiple records creation simultaneously', async () => {
            ...
            await INSERT.into(Items).entries([
                { ID: id1, name: 'Item 1', quantity: 100 },
                { ID: id2, name: 'Item 2', quantity: 200 },
                { ID: id3, name: 'Item 3', quantity: 300 }
            ]);
            ...
            const relevantChanges = changes.filter(c => [id1, id2, id3].includes(c.entityKey));
            expect(relevantChanges.length).toEqual(3);
            expect(relevantChanges.find(c => c.entityKey === id1)).toMatchObject({ valueChangedTo: '100' });
            expect(relevantChanges.find(c => c.entityKey === id2)).toMatchObject({ valueChangedTo: '200' });
            expect(relevantChanges.find(c => c.entityKey === id3)).toMatchObject({ valueChangedTo: '300' });
        });
        ...
        it('should track numeric value 0 on create', async () => {
            ...
            await INSERT.into(Items).entries({ ID: id, name: 'Zero Quantity Item', quantity: 0 });
    ...
    describe('Update Tracking', () => {
        ...
        it('should track update from non-null to null value', async () => {
            ...
            await UPDATE(Items).where({ ID: id }).with({ category: null });
        ...
        it('should track update from null to non-null value', async () => {
            ...
            await INSERT.into(Items).entries({ ID: id, name: 'No Category', category: null });

            await UPDATE(Items).where({ ID: id }).with({ category: 'New Category' });
    ...
    describe('Delete Tracking', () => {
        ...
        it('should track numeric value 0 on delete', async () => {
            ...
            await INSERT.into(Items).entries({ ID: id, name: 'Zero Quantity', quantity: 0 });
            await DELETE.from(Items).where({ ID: id });
