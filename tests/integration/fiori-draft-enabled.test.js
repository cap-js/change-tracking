...
describe("change log integration test", () => {
    ...
    it("2.4 Child entity update without objectID annotation - should log object type for object ID (ERP4SMEPREPWORKAPPPLAT-32 ERP4SMEPREPWORKAPPPLAT-613 ERP4SMEPREPWORKAPPPLAT-538)", async () => {
        delete cds.services.AdminService.entities.Books["@changehistory.objectID"];
        delete cds.services.AdminService.entities.BookStores["@changehistory.objectID"];
        delete cds.db.entities.Books["@changehistory.objectID"];
        delete cds.db.entities.BookStores["@changehistory.objectID"];
        ...
        cds.services.AdminService.entities.Books["@changehistory.objectID"] = [
            { "=": "title" },
            { "=": "author.name.firstName" },
            { "=": "author.name.lastName" },
        ];
        cds.services.AdminService.entities.BookStores["@changehistory.objectID"] = [
            {
                "=": "name",
            },
        ];
    ...
    it("4.1 Annotate multiple native and attributes comming from one or more associated table as the object ID (ERP4SMEPREPWORKAPPPLAT-913)", async () => {
        ...
        cds.services.AdminService.entities.Books["@changehistory.objectID"].push(
            { "=": "stock" },
            { "=": "bookStore.name" },
            { "=": "bookStore.location" }
        );
    ...
    it("5.1 Value data type records data type of native attributes of the entity or attributes from association table which are annotated as the displayed value(ERP4SMEPREPWORKAPPPLAT-873)", async () => {
        cds.services.AdminService.entities.Books.elements.author_ID["@changehistory"] = [
            { "=": "author.name.firstName" },
            { "=": "author.dateOfBirth" },
            { "=": "author.name.lastName" },
        ];

        cds.services.AdminService.entities.Books.elements.author["@changehistory"] = [
            { "=": "author.name.firstName" },
            { "=": "author.dateOfBirth" },
            { "=": "author.name.lastName" },
        ];
        ...
        // recover @changehistory annotation on the association table author
        cds.services.AdminService.entities.Books.elements.author_ID["@changehistory"] = [
            { "=": "author.name.firstName" },
            { "=": "author.name.lastName" },
        ];
    ...
    it("5.2 Value data type records data type of native attributes of the entity or attributes from composition which are annotated as the displayed value (ERP4SMEPREPWORKAPPPLAT-873)", async () => {
        cds.services.AdminService.entities.BookStores.elements.books["@changehistory"] = [
            { "=": "books.title" },
            { "=": "books.stock" },
            { "=": "books.price" },
        ];
        ...
        // recover @changehistory context on composition books
        cds.services.AdminService.entities.BookStores.elements.books["@changehistory"] = [{ "=": "books.title" }];
    });
    ...
    it("6.3 Attributes from the code list could be annotated as object ID (ERP4SMEPREPWORKAPPPLAT-1055)", async () => {
        ...
        cds.services.AdminService.entities.BookStores["@changehistory.objectID"] = [{ "=": "name" }];
    });
