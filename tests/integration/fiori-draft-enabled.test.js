...
describe("change log integration test", () => {
    ...
    it("2.1 Child entity creation - should log basic data type changes (ERP4SMEPREPWORKAPPPLAT-32 ERP4SMEPREPWORKAPPPLAT-613)", async () => {
        ...
        const unmanagedAction = POST.bind(
            {},
            `/admin/Schools(ID=5ab2a87b-3a56-4d97-a697-7af72333c123,IsActiveEntity=false)/classes`,
            {
    ...
    it("2.2 Child entity update - should log basic data type changes (ERP4SMEPREPWORKAPPPLAT-32 ERP4SMEPREPWORKAPPPLAT-613)", async () => {
        ...
        // Test for Unmanaged entity(Update)
        const unmanagedAction = PATCH.bind(
            {},
            `/admin/Schools_classes(up__ID=5ab2a87b-3a56-4d97-a697-7af72333c123,ID=9d703c23-54a8-4eff-81c1-cdec5a0422c3,IsActiveEntity=false)`,
            {
                name: "Math 300",
                teacher: "Mr. Dylan"
            }
        ...
        expect(schoolChange.entityKey).to.equal("5ab2a87b-3a56-4d97-a697-7af72333c123");
        expect(schoolChange.attribute).to.equal("classes");
        expect(schoolChange.modification).to.equal("Update");
        expect(schoolChange.valueChangedFrom).to.equal("Math 300, Mr. Dylan");
        expect(schoolChange.valueChangedTo).to.equal("Math 300, Mr. Dylan");
    });
    ...
    it("2.3 Child entity delete - should log basic data type changes (ERP4SMEPREPWORKAPPPLAT-32 ERP4SMEPREPWORKAPPPLAT-613)", async () => {
        ...
        const unmanagedAction = DELETE.bind({},`/admin/Schools_classes(up__ID=5ab2a87b-3a56-4d97-a697-7af72333c123,ID=9d703c23-54a8-4eff-81c1-cdec5a0422c3,IsActiveEntity=false)`);
        await utils.apiAction("admin", "Schools", "5ab2a87b-3a56-4d97-a697-7af72333c123", "AdminService", unmanagedAction);
        const schoolChanges = await adminService.run(
            SELECT.from(ChangeView).where({
                entity: "sap.capire.bookshop.Schools",
