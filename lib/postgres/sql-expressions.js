...
function buildObjectIDAssignment(objectIDs, entity, keys, recVar, targetVar, model) {
	...
	for (const oid of objectIDs) {
		if (oid.included) {
			...
			nullChecks.push(`${recVar}.${quote(oid.name)} IS NULL`);
	...
	if (objectIDs.length === 1) {
		const valueExpr = objectIDs[0].included ? `${recVar}.${quote(objectIDs[0].name)}::TEXT` : parts[0];
		return `${targetVar} := COALESCE(${valueExpr}, entity_key);`;
	}

	// Multiple objectID fields: use CONCAT_WS with '<empty>' for individual NULLs,
	// fall back to entityKey when ALL fields are NULL
	const allNullCondition = nullChecks.join(' AND ');

	return `
    IF ${allNullCondition} THEN
