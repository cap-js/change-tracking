...
function buildCompositionParentBlock(compositionParentInfo, rootObjectIDs, modification, model, ancestorCompositionChain = [], childObjectIDExpr = null, compositionFieldObjectIDExpr = null) {
	const { parentEntityName, compositionFieldName, parentKeyBinding } = compositionParentInfo;
	...
	const parentKeyExpr = entityKeyExpr(parentKeyBinding.map((k) => `rec.${quote(k)}`));
	...
	const immediateParentObjectIDExpr = childObjectIDExpr ?? compositionFieldObjectIDExpr ?? buildCompOfManyRootObjectIDSelect(rootEntity, rootObjectIDs, parentKeyBinding, 'rec', model);
	...
	if (ancestorCompositionChain.length > 0) {
		...
		const ancestorKeyValues = [parentKeyBinding.map((k) => `rec.${quote(k)}`)]; // level 0
		...
		childWhereClauses.push(childKeys0.map((pk, j) => `${quote(pk)} = rec.${quote(levels[0].keyBinding[j])}`).join(' AND '));
