 ...
 * Resolves @changelog objectIDs from the parent table; falls back to the entity-key string
 * when no objectID is configured. With multiple objectIDs uses CONCAT_WS.
 *
 ...
 * @param {string} refRow - Trigger row reference (e.g. 'rec', 'new', 'old')
...
/**
 * Builds the PL/pgSQL block that inserts a composition-of-one parent changelog entry.
 * Performs a reverse lookup (parent has FK to child) to resolve the parent's key,
 * and uses idempotent INSERT-IF-NOT-EXISTS semantics keyed on the transaction.
 *
 ...
 * @returns {string} A PL/pgSQL block (within an `IF EXISTS ... THEN ... END IF;`)
 */
...
/**
 * Builds the PL/pgSQL block that inserts composition parent entries for a Postgres trigger.
 * Emits one INSERT per level from outermost ancestor down to the immediate parent,
 * using `comp_parent_id` / `comp_ancestor_<i>_id` variables to chain parent_ID links.
 *
 ...
 * @param {object} model - The CDS model (CSN)
 * @param {string|null} [childObjectIDExpr] - SQL expression for the trigger entity's objectID
 ...
 * @returns {string} A PL/pgSQL block
 */
