...
const { getSkipCheckCondition, entityKeyExpr, buildObjectIDAssignment, buildInsertBlock, extractTrackedDbColumns } = require('./sql-expressions.js');
...
function buildCompositionFieldObjectID(compositionFieldChangelog, parentEntityName, parentEntity, parentKeyBinding, model) {
	const parsed = parseCompositionFieldChangelog(compositionFieldChangelog, parentEntity, parentKeyBinding, 'rec');
...
function buildFunctionBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, ancestorCompositionChain = []) {
	...
	const entityKey = entityKeyExpr(keys.map((k) => `rec.${k}`));
