...
/**
 * Generates the PL/pgSQL function body for an entity's change tracking trigger.
 * Picks the trigger row (NEW or OLD), assigns objectID, and emits the composition parent
 * block plus per-modification INSERT blocks gated by `TG_OP`.
 *
 * @param {object} entity - The CDS entity definition
 * @param {Array} columns - Tracked columns descriptors
 * @param {Array} objectIDs - @changelog objectIDs of the entity
 * @param {Array} parentObjectIDs - @changelog objectIDs of the immediate parent (if any)
 * @param {object} model - The CDS model (CSN)
 * @param {object|null} [compositionHierarchy] - Composition hierarchy as returned by `getCompositionHierarchy`
 * @returns {string} The DECLARE...BEGIN...END function body
 */
function buildFunctionBody(entity, columns, objectIDs, parentObjectIDs, model, compositionHierarchy = null) {
  ...
  const parentBlock = compositionHierarchy
    ? buildCompositionParentBlock(compositionHierarchy, parentObjectIDs, model, 'object_id', (changelog, parentEntity, keyBinding) => buildCompositionFieldObjectID(changelog, parentEntity, keyBinding, model))
    : '';
...
/**
 * Generates Postgres CREATE FUNCTION + CREATE TRIGGER statements for an entity.
 ...
 * @param {object|null} parentEntity - The immediate parent entity (composition owner), if any
 ...
 * @returns {string[]} Array of SQL statements (function + trigger), or empty array
 */
function generatePostgresTriggers(csn, entity, parentEntity, mergedAnnotations = null, parentMergedAnnotations = null, grandParentContext = {}) {
  ...
  const funcBody = buildFunctionBody(entity, trackedColumns, objectIDs, parentObjectIDs, csn, compositionHierarchy);
