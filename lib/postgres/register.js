...
function registerPostgresCompilerHook() {
	...
	cds.on('serving', async () => {
		...
		db.before('*', () => {
			...
			db.class.CQN2SQL.OutputConverters.DateTime = (e) => `to_timestamp(${e}::text, 'YYYY-MM-DD"T"HH24:MI:SS"Z"')`;
			db.class.CQN2SQL.OutputConverters.Timestamp = (e) => `to_timestamp(${e}::text, 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')`;
		});
	});
}
