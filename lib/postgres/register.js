...
/**
 * Registers a handler on the DeploymentService to deploy service-level
...
function registerPostgresDeploymentHandler() {
  cds.on('serving:cds.xt.DeploymentService', (ds) => {
    ds.after('deploy', async (_, req) => {
      ...
      // Work on a structuredClone so that subsequent enhanceModel /
      // compile.for.nodejs / linker mutations do not pollute the cached CSN.
