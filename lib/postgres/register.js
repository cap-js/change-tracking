...
function registerPostgresCompilerHook() {
	...
	cds.on('serving', async () => {
		...
		db.before('*', () => {
			db.class.CQN2SQL.OutputConverters.Date = (e) => `to_date(${e}, 'YYYY-MM-DD')`;
			db.class.CQN2SQL.OutputConverters.Time = (e) => `to_timestamp(${e}, 'HH24:MI:SS')::TIME`;
			db.class.CQN2SQL.OutputConverters.DateTime = (e) => `to_timestamp(${e}, 'YYYY-MM-DD"T"HH24:MI:SS"Z"')`;
			db.class.CQN2SQL.OutputConverters.Timestamp = (e) => `to_timestamp(${e}, 'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"')`;
		});
	});
}
