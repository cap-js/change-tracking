...
function registerH2CompilerHook() {
  cds.on('compile.to.dbx', (csn, options, next) => {
    ...
    _writeTriggerJavaFiles(javaClasses, options);
 ...
 */
function _writeTriggerJavaFiles(javaClasses, options) {
  ...
  if (!options?.dry) LOG.info(`Wrote ${javaClasses.length + 1} H2 trigger Java file(s) to ${outputDir}`);
}
