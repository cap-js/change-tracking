...
function _buildChangelogInsertSQL(col, hasParent, fromExpr, toExpr, fromLabelExpr, toLabelExpr, modification) {
  ...
  const values = hasParent
    ? `(RANDOM_UUID(), ?, '${col.name}', ${fromExpr}, ${toExpr}, ${fromLabelExpr}, ${toLabelExpr}, ?, ?, ?, CURRENT_TIMESTAMP(), ?, '${valueType}', '${modification}', TRANSACTION_ID())`
    : `(RANDOM_UUID(), '${col.name}', ${fromExpr}, ${toExpr}, ${fromLabelExpr}, ${toLabelExpr}, ?, ?, ?, CURRENT_TIMESTAMP(), ?, '${valueType}', '${modification}', TRANSACTION_ID())`;
...
function _generateNonUpdateColumnBlock(col, reference, compositionParentInfo, entity, model, modification) {
  ...
  const allBindings = compositionParentInfo ? ['parentId', ...bindings, ...labelRes.bindings, 'entityName', 'entityKey', objectIDVar, 'createdBy'] : [...bindings, ...labelRes.bindings, 'entityName', 'entityKey', objectIDVar, 'createdBy'];
  ...
  const columnBlocks = columns
    .map((col) => {
      ...
      const allBindings = compositionParentInfo
        ? ['parentId', ...oldRes.bindings, ...newRes.bindings, ...oldLabelRes.bindings, ...newLabelRes.bindings, 'entityName', 'entityKey', objectIDVar, 'createdBy']
        : [...oldRes.bindings, ...newRes.bindings, ...oldLabelRes.bindings, ...newLabelRes.bindings, 'entityName', 'entityKey', objectIDVar, 'createdBy'];
