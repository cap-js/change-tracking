...
function _generateJavaMethod(createBody, updateBody, deleteBody, entityName, compositionParentInfo = null, grandParentCompositionInfo = null, ancestorLevels = []) {
  ...
  const ancestorHelpers = ancestorLevels.length > 0
    ? _generateAncestorHelpers(ancestorLevels)
    : (grandParentCompositionInfo
        ? _generateGrandParentHelper(
            grandParentCompositionInfo.grandParentEntityName,
            grandParentCompositionInfo.grandParentCompositionFieldName
          )
        : '');
...
function _generateNonUpdateColumnBlock(col, reference, compositionParentInfo, entity, model, modification) {
  ...
  const allBindings = compositionParentInfo
    ? ['parentId', ...bindings, ...labelRes.bindings, 'entityName', 'entityKey', objectIDVar]
    : [...bindings, ...labelRes.bindings, 'entityName', 'entityKey', objectIDVar];
...
function _generateNonUpdateColumnBlocks(columns, reference, compositionParentInfo, entity, model, modification) {
  return columns
    .map((col) => _generateNonUpdateColumnBlock(col, reference, compositionParentInfo, entity, model, modification))
    .join('\n');
}
...
function _generateCreateBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, grandParentCompositionInfo = null, ancestorChain = null) {
  ...
  const { modificationTypeSetup, keysCalc, parentIdSetup } = _buildBodyPrelude(
    entity, columns, objectIDs, rootEntity, rootObjectIDs, model,
    compositionParentInfo, grandParentCompositionInfo, ancestorChain, reference, 'create'
  );
...
function _generateUpdateBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, grandParentCompositionInfo = null, ancestorChain = null) {
  ...
  const { modificationTypeSetup, keysCalc, parentIdSetup } = _buildBodyPrelude(
    entity, columns, objectIDs, rootEntity, rootObjectIDs, model,
    compositionParentInfo, grandParentCompositionInfo, ancestorChain, reference, 'update'
  );
  ...
  const columnBlocks = columns
    .map((col) => {
      ...
      const insertSQL = _buildChangelogInsertSQL(
        col, !!compositionParentInfo,
        oldRes.sqlExpr, newRes.sqlExpr, oldLabelRes.sqlExpr, newLabelRes.sqlExpr,
        'update'
      );
...
function _generateDeleteBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, grandParentCompositionInfo = null, ancestorChain = null) {
  ...
  const { modificationTypeSetup, keysCalc, parentIdSetup } = _buildBodyPrelude(
    entity, columns, objectIDs, rootEntity, rootObjectIDs, model,
    compositionParentInfo, grandParentCompositionInfo, ancestorChain, reference, 'delete'
  );

  // First delete existing changelogs for this entity
  const deleteBlock = _wrapInTryCatch(
    `DELETE FROM sap_changelog_Changes WHERE ENTITY = ? AND ENTITYKEY = ?`,
