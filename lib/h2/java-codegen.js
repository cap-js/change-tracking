...
/**
 * Builds a scalar subquery that evaluates a CDS expression on the entity.
 * For H2 triggers, key references use parameterized queries with ?.
 */
function _buildExpressionSubquery(entityName, exprColumn, keys, rowVar, model) {
	const where = keys.reduce((acc, k) => {
		acc[k] = { ref: ['?'], param: true };
		return acc;
	}, {});
	const query = SELECT.one.from(entityName).columns(exprColumn).where(where);
	const sql = `(${_toSQL(query, model)})`;
	const bindings = keys.map((k) => `${rowVar}.getString("${k}")`);
	return { sql, bindings };
}
...
function _generateCreateBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, grandParentCompositionInfo = null) {
	...
	const columnBlocks = columns
		.map((col) => {
			...
			const labelRes = _prepareLabelExpression(col, reference, model, entity.name); // label expression
            ...
            VALUES 
            (RANDOM_UUID(), ?, '${col.name}', NULL, ${sqlExpr}, NULL, ${labelRes.sqlExpr}, ?, ?, ?, CURRENT_TIMESTAMP(), CURRENT_USER(), '${col.altExpressions ? 'cds.String' : col.type}', 'create', TRANSACTION_ID())`
				: `INSERT INTO sap_changelog_Changes 
...
function _generateUpdateBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, grandParentCompositionInfo = null) {
	...
	const columnBlocks = columns
		.map((col) => {
			...
			const newLabelRes = _prepareLabelExpression(col, 'newRow', model, entity.name);
			const oldLabelRes = _prepareLabelExpression(col, 'oldRow', model, entity.name);
            ...
            (ID, PARENT_ID, ATTRIBUTE, VALUECHANGEDFROM, VALUECHANGEDTO, VALUECHANGEDFROMLABEL, VALUECHANGEDTOLABEL, ENTITY, ENTITYKEY, OBJECTID, CREATEDAT, CREATEDBY, VALUEDATATYPE, MODIFICATION, TRANSACTIONID) 
            VALUES 
            (RANDOM_UUID(), ?, '${col.name}', ${oldRes.sqlExpr}, ${newRes.sqlExpr}, ${oldLabelRes.sqlExpr}, ${newLabelRes.sqlExpr}, ?, ?, ?, CURRENT_TIMESTAMP(), CURRENT_USER(), '${col.altExpressions ? 'cds.String' : col.type}', 'update', TRANSACTION_ID())`
				: `INSERT INTO sap_changelog_Changes 
            (ID, ATTRIBUTE, VALUECHANGEDFROM, VALUECHANGEDTO, VALUECHANGEDFROMLABEL, VALUECHANGEDTOLABEL, ENTITY, ENTITYKEY, OBJECTID, CREATEDAT, CREATEDBY, VALUEDATATYPE, MODIFICATION, TRANSACTIONID) 
            VALUES 
            (RANDOM_UUID(), '${col.name}', ${oldRes.sqlExpr}, ${newRes.sqlExpr}, ${oldLabelRes.sqlExpr}, ${newLabelRes.sqlExpr}, ?, ?, ?, CURRENT_TIMESTAMP(), CURRENT_USER(), '${col.altExpressions ? 'cds.String' : col.type}', 'update', TRANSACTION_ID())`;
...
function _generateDeleteBody(entity, columns, objectIDs, rootEntity, rootObjectIDs, model, compositionParentInfo = null, grandParentCompositionInfo = null) {
	...
	const columnBlocks = columns
		.map((col) => {
			...
			const labelRes = _prepareLabelExpression(col, reference, model, entity.name);
            ...
            (ID, PARENT_ID, ATTRIBUTE, VALUECHANGEDFROM, VALUECHANGEDTO, VALUECHANGEDFROMLABEL, VALUECHANGEDTOLABEL, ENTITY, ENTITYKEY, OBJECTID, CREATEDAT, CREATEDBY, VALUEDATATYPE, MODIFICATION, TRANSACTIONID) 
            VALUES 
            (RANDOM_UUID(), ?, '${col.name}', ${sqlExpr}, NULL, ${labelRes.sqlExpr}, NULL, ?, ?, ?, CURRENT_TIMESTAMP(), CURRENT_USER(), '${col.altExpressions ? 'cds.String' : col.type}', 'delete', TRANSACTION_ID())`
				: `INSERT INTO sap_changelog_Changes 
            (ID, ATTRIBUTE, VALUECHANGEDFROM, VALUECHANGEDTO, VALUECHANGEDFROMLABEL, VALUECHANGEDTOLABEL, ENTITY, ENTITYKEY, OBJECTID, CREATEDAT, CREATEDBY, VALUEDATATYPE, MODIFICATION, TRANSACTIONID) 
            VALUES 
            (RANDOM_UUID(), '${col.name}', ${sqlExpr}, NULL, ${labelRes.sqlExpr}, NULL, ?, ?, ?, CURRENT_TIMESTAMP(), CURRENT_USER(), '${col.altExpressions ? 'cds.String' : col.type}', 'delete', TRANSACTION_ID())`;
...
function _prepareLabelExpression(col, rowVar, model, entityName = null) {
	// Expression-based labels: build SQL with trigger row refs for local columns
	// and subqueries for association paths
	if (col.altExpressions && col.altExpressions.length > 0 && entityName) {
		const entity = model.definitions[entityName];
		if (entity) {
			const sqlParts = [];
			const allBindings = [];

			for (const xpr of col.altExpressions) {
				const { sql, bindings } = _buildExpressionSQLForH2(xpr, entityName, rowVar, model);
				sqlParts.push(sql);
...
/**
 * Translates a CDS expression to H2 SQL with parameterized bindings.
 * Uses CQN2SQL's built-in expression renderer with an overridden ref() method.
 * Local refs become ? with Java ResultSet bindings.
 * Association paths become scalar subqueries with parameterized FK values.
 */
function _buildExpressionSQLForH2(xpr, entityName, rowVar, model) {
	const entity = model.definitions[entityName];
	const SQLiteService = require('@cap-js/sqlite');
	const allBindings = [];

	const renderer = new SQLiteService.CQN2SQL({ model });
	renderer.ref = function ({ ref }) {
		if (ref.length === 1) {
			allBindings.push(`${rowVar}.getString("${ref[0]}")`);
			return '?';
		}
		...
		if (assocElement?.target) {
			...
			if (assocElement.keys) {
				for (const k of assocElement.keys) {
					...
					where[fkName] = { ref: ['?'], param: true };
					allBindings.push(`${rowVar}.getString("${assocName}_${fkName}")`);
				}
			}
			const query = SELECT.one.from(assocElement.target).columns(fieldPath).where(where);
			return `(${_toSQL(query, model)})`;
		}
		allBindings.push(`${rowVar}.getString("${ref.join('_')}")`);
		return '?';
	};

	const sql = renderer.xpr({ xpr });
...
function _generateObjectIDCalculation(objectIDs, entity, refRow, model) {
	...
	for (const oid of objectIDs) {
		if (oid.included) {
			...
			const { sql, bindings: exprBindings } = _buildExpressionSubquery(entity.name, exprColumn, keys, refRow, model);
			parts.push(`SELECT CAST(${sql} AS VARCHAR) AS val`);
			bindings.push(...exprBindings);
		} else {
