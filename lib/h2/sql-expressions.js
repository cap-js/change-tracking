...
const _NUMERIC_TYPES = new Set(['cds.Decimal', 'cds.DecimalFloat', 'cds.Double', 'cds.Integer', 'cds.Integer64', 'cds.UInt8', 'cds.Int16', 'cds.Int32', 'cds.Int64']);
...
function _wrapInTryCatch(sql, bindings) {
  const cleanSql = _escapeForJavaStringLiteral(sql).replace(/\n/g, ' ');
...
/**
 * Escapes a string for embedding as a Java double-quoted string literal.
 *
 * Backslashes must be escaped BEFORE double quotes; otherwise the `\"` produced by
 * the quote-escape step would itself get double-escaped to `\\"` by the backslash
 * step, corrupting valid quotes. In our current use-cases the input never contains
 * backslashes (SQL identifiers and CDS entity names cannot), but doing it correctly
 * silences CodeQL's `js/incomplete-sanitization` alert and prevents future breakage
 * if that assumption ever changes.
 */
function _escapeForJavaStringLiteral(s) {
  return s.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
}

function entityKeyExpr(parts) {
  if (parts.length <= 1) return `CAST(${parts[0]} AS VARCHAR)`;
  return parts.map((p) => `LENGTH(CAST(${p} AS VARCHAR)) || ',' || CAST(${p} AS VARCHAR)`).join(" || ';' || ");
}
