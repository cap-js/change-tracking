...
function _generateParentObjectIDCalculation(rootObjectIDs, rootEntity, refRow, childEntity, model, parentKeyBinding = null) {
  if (!rootObjectIDs || rootObjectIDs.length === 0) {
    // fallback to parent's key when objectID on the parent
  ...
  for (const oid of rootObjectIDs) {
    ...
    if (isCompositionOfOne) {
      ...
      where = binding.childKeys.reduce((acc, ck) => {
        acc[`${binding.compositionName}_${ck}`] = { val: '?', literal: 'sql' };
        return acc;
      }, {});
    } else {
    ...
    } else if (Array.isArray(parentKeyBinding) && parentKeyBinding.length > 0) {
