			...
			if (latestVersion >= 2) {
				LOG.warn(
					`Migration table already exists at ${MIGRATION_TABLE_PATH} (latest version: ${latestVersion}). ` +
						`Only the initial migration is supported by this command. ` +
						`Please add new migration steps manually.`
				);
				return;
			}

			// Append v2 version and migration to existing file
			fs.appendFileSync(MIGRATION_TABLE_PATH, '\n' + getMigrationTableSQL());
			LOG.info(`Appended v2 migration to existing ${MIGRATION_TABLE_PATH}`);
		} else {
