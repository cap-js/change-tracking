...
function entityKey4(entity, model) {
	...
	for (const k in entity.elements) {
		...
		if (e.type === 'cds.Association') {
			...
			keys.push({ ref: [k, foreignKeyName] });
			const targetEntity = e._target || model?.definitions?.[e.target];
			keyTypes.push(targetEntity?.elements?.[foreignKeyName]?.type);
		} else {
			keys.push({ ref: [k] });
			keyTypes.push(e.type);
		}
	}

	const needsCast = (type) => type !== 'cds.String' && type !== 'cds.UUID';
	const asStr = (key, i) => needsCast(keyTypes[i]) ? { ...key, cast: { type: 'cds.String' } } : key;

	if (keys.length <= 1) return keys.map((k, i) => asStr(k, i));
