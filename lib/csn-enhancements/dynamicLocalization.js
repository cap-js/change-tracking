  ...
  for (const name in m.definitions) {
    ...
    for (const ele in entity.elements) {
      ...
      if (!dynamicLocalizationProperties.some((prop) => prop.property === basePropertyName && prop.entity === base.baseRef) && m.definitions[textTarget]) {
        ...
        LOG.warn(`Cannot dynamically localize "${ele}" of ${name} because ${textTarget} is not defined in the CDS model. This might be because the "texts" property is not part of the projection.`);
...
function enhanceChangeViewWithLocalization(serviceName, changeViewName, m) {
  ...
  for (const localizationProp of localizationProperties) {
    ...
    subSelect.SELECT.where = [
      { ref: [changeViewCqn.from.as, 'valueChangedFrom'] },
      '=',
      { cast: { type: 'cds.String' }, xpr: [{ ref: ['localizationSubSelect', keys[0]] }] },
      'and',
      { ref: ['localizationSubSelect', 'locale'] },
      '=',
      { ref: ['$user', 'locale'] }
    ];
    ...
    subSelect2.SELECT.where = [
      { ref: [changeViewCqn.from.as, 'valueChangedTo'] },
      '=',
      { cast: { type: 'cds.String' }, xpr: [{ ref: ['localizationSubSelect', keys[0]] }] },
      'and',
      { ref: ['localizationSubSelect', 'locale'] },
