...
function collectTrackedPropertiesWithDynamicLocalization(serviceName, m) {
	...
	for (const name in m.definitions) {
		...
		for (const ele in entity.elements) {
			...
			if (typeof changelogEntry['='] !== 'string' || changelogEntry.xpr) {
			...
			const segments = changelogEntry['='].split('.');
			...
			if (!target || (segments[0] !== basePropertyName && !basePropertyInUnmanagedOnCondition) || segments.length !== 2 || !isLocalizedField || amountOfKeys > 1) {
				DEBUG?.(`Dynamic localization lookup is not performed on ${ele} of ${name} for the path "${element['@changelog'][0]['=']}". Only paths which follow the properties association, which only navigate one level deep and where the last property is localized are supported.`);
				continue;
			}
