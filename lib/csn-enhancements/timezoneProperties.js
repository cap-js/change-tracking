...
function enhanceChangeViewWithTimeZones(changeView, m) {
    const compositeKeyExpr = isDeploy2Check('hana') ? compositeKeyExprHANA : isDeploy2Check('postgres') ? compositeKeyExprPG
        : cds.env.requires?.db.kind === 'sqlite' ? compositeKeyExprSqlite : cds.env.requires?.db.kind === 'postgres' ? compositeKeyExprPG : compositeKeyExprHANA;
