...
/*const targetEntity = cds.entities[template.target.elements[key].target];
                subRow._path = [...row._path, {target: serviceNodeName, key: Object.fromEntries(keys.map(k => [k.name, subRow[k.name]]))}];
                const keys = targetEntity.keys.filter(k => k.type !== "cds.Association").filter(k => !k.virtual);*/
    ...
    if (subTemplate) {
        ...
        subRows.forEach((subRow) => {
            if (subRow && row && row._path) {
                ...
                subRow._path = [...row._path, {target: serviceNodeName, key: {ID: subRow.ID}}];
            }
        });
