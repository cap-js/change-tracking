...
function generateTriggersForEntity(csn, entity) {
	...
	if (!config?.disableCreateTracking) {
		const declaration = _generateTriggerDeclaration(entity, 'new', null, objectIDs, null);
		triggers.push(_generateCreateTrigger(entity, trackedColumns, declaration, objectIDs));
	}

	if (!config?.disableUpdateTracking) {
		const declaration = _generateTriggerDeclaration(entity, 'old', null, objectIDs, null);
		triggers.push(_generateUpdateTrigger(entity, trackedColumns, declaration, objectIDs));
	}

	if (!config?.disableDeleteTracking) {
		if (config?.preserveDeletes) {
			const declaration = _generateTriggerDeclaration(entity, 'old', null, objectIDs, null);
			triggers.push(_generateDeleteTriggerPreserve(entity, trackedColumns, declaration, objectIDs));
		} else {
...
function _generateTriggerDeclaration(entity, rowRef, rootEntity, objectID, rootObjectID) {
	// Entity Keys
...
function _generateUpdateTrigger(entity, columns, triggerDeclaration, objectIDs) {
	const keys = utils.extractKeys(entity.keys);
...
function _generateCreateTrigger(entity, columns, triggerDeclarations, objectIDs) {
	const keys = utils.extractKeys(entity.keys);
...
function _generateDeleteTriggerPreserve(entity, columns, triggerDeclarations, objectIDs) {
	const keys = utils.extractKeys(entity.keys);
	const objectID = _getObjectIDSelect(objectIDs, entity.name, keys, 'old');
