...
const _localizeValue = (change, locale) => {
    ...
    const options = formatOptions[change.valueDataType]?.[normalizedLocale] || formatOptions[change.valueDataType]?.[defaultLocale];
    ...
    if (change.valueDataType === 'cds.Time') {
        const parseTime = (time) => {
            ...
            return date.toLocaleTimeString(normalizedLocale, options);
        };
        if (change.valueChangedFrom) change.valueChangedFrom = parseTime(change.valueChangedFrom);
        if (change.valueChangedTo) change.valueChangedTo = parseTime(change.valueChangedTo);
    } else {
