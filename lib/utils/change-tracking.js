...
function extractTrackedColumns(entity, model = cds.model, overrideAnnotations = null) {
	...
	for (const [name, col] of Object.entries(entity.elements)) {
		...
		const hasChildSideFK =
			col.type === 'cds.Composition' && (col.is2many || (col.on && col.on.some((c) => c.ref?.includes('$self'))));
		if (hasChildSideFK) {
			const compEntry = {
				name: name,
				target: col.target
...
function getLocalizedLookupInfo(targetEntityName, altFields, model = cds.model) {
