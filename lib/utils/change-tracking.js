const cds = require('@sap/cds');
const LOG = cds.log('change-tracking');

function isExpression(annotationValue) {
  return annotationValue && !!annotationValue.xpr;
}

/**
 * Recursively collect the first segment (ref[0]) of all references
 * found in a CDS expression (Array-based AST as produced by the compiler).
 *
 * @param {Array} xpr - CDS expression array
 * @param {Set<string>} [seenRefs] - internal accumulator
 * @returns {Set<string>} the set of unique top-level element names referenced
 */
function collectRefsFromXpr(xpr, seenRefs = new Set()) {
  for (const token of xpr) {
    if (token.ref) {
      const id = token.ref[0].id || token.ref[0];
      seenRefs.add(id);
      if (token.ref[0].where) collectRefsFromXpr(token.ref[0].where, seenRefs);
    } else if (token.xpr) {
      collectRefsFromXpr(token.xpr, seenRefs);
    } else if (token.args) {
      collectRefsFromXpr(token.args, seenRefs);
    } else if (token.list) {
      token.list.forEach((item) => collectRefsFromXpr(item, seenRefs));
    }
  }
  return seenRefs;
}

/**
 * REVISIT: maybe use q.forSQL() and check if subselects or joins are used
 * Validates that all ref tokens in a CDS expression reference elements on the parent entity, not the composition field's target entity.
 */
function _validateExpressionRefs(xpr, compositionFieldName, parentEntity) {
  if (!Array.isArray(xpr)) return true;
  for (const id of collectRefsFromXpr(xpr)) {
    // Reject refs that start with the composition field name (referencing child entity)
    if (id === compositionFieldName) return false;
    // Validate the ref exists on the parent entity
    if (!parentEntity.elements?.[id]) return false;
  }
  return true;
}

/**
 * Checks whether an element is annotated with @PersonalData.
 */
function _hasPersonalData(element) {
  return !!(element?.['@PersonalData.IsPotentiallyPersonal'] || element?.['@PersonalData.IsPotentiallySensitive']);
}

/**
 * Checks whether any ref token in a CDS expression resolves to a @PersonalData field.
 * Returns true if personal data is referenced, false otherwise.
 * Recurses into nested sub-expressions (token.xpr) so that refs buried inside
 * parenthesized expressions are not missed.
 */
function _xprRefsPersonalData(xpr, entity, model) {
  if (!Array.isArray(xpr)) return false;
  for (const token of xpr) {
    if (token?.xpr) {
      if (_xprRefsPersonalData(token.xpr, entity, model)) return true;
    }
    if (token?.args) {
      if (_xprRefsPersonalData(token.args, entity, model)) return true;
    }
    if (token?.list) {
      if (token.list.some((item) => _xprRefsPersonalData(item, entity, model))) return true;
    }
    if (!token?.ref?.length) continue;
    let currentEntity = entity;
    const segments = token.ref;
    for (let i = 0; i < segments.length; i++) {
      const element = currentEntity.elements?.[segments[i]];
      if (!element) break;
      if (i === segments.length - 1) {
        if (_hasPersonalData(element)) return true;
      } else if (element.target) {
        const targetDef = model.definitions[element.target];
        if (!targetDef) break;
        currentEntity = targetDef;
      }
    }
  }
  return false;
}

/**
 * Method to validate changelog path on entity
 * Normalizes flattened paths too
 * @param {*} entity CSN entity definition
 * @param {*} path provided changelog path
 * @returns normalized path or null if invalid
 */
function validateChangelogPath(entity, path, model = cds.context?.model ?? cds.model) {
  const segments = path.split('.');

  if (segments.length === 1) {
    const element = entity.elements?.[segments[0]];
    if (!element) return null;
    if (_hasPersonalData(element)) {
      LOG.warn(`Skipped @changelog path '${path}' on entity '${entity.name}': Target element is annotated with @PersonalData.`);
      return null;
    }
    return segments[0];
  }

  let currentEntity = entity;
  const walked = [];

  for (let i = 0; i < segments.length; i++) {
    const seg = segments[i];
    const element = currentEntity.elements?.[seg];
    if (!element) {
      // try flattened tail on current entity
      const flattened = segments.slice(i).join('_');
      const flatElement = currentEntity.elements?.[flattened];
      if (flatElement) {
        if (_hasPersonalData(flatElement)) {
          LOG.warn(`Skipped @changelog path '${path}' on entity '${entity.name}': Target element is annotated with @PersonalData.`);
          return null;
        }
        walked.push(flattened);
        return walked.join('.');
      }
      LOG.warn(`Invalid @changelog path '${path}' on entity '${entity.name}': '${seg}' not found. @changelog skipped.`);
      return null;
    }
    walked.push(seg);

    if ((element.type === 'cds.Association' || element.type === 'cds.Composition' || element.type === 'Country') && element.target) {
      const targetDef = model.definitions[element.target];
      if (!targetDef || targetDef.kind !== 'entity') {
        return null;
      }
      currentEntity = targetDef;
      continue;
    }

    // Check primitive field
    if (i === segments.length - 1) {
      if (_hasPersonalData(element)) {
        LOG.warn(`Skipped @changelog path '${path}' on entity '${entity.name}': Target element is annotated with @PersonalData.`);
        return null;
      }
      return walked.join('.');
    }
  }
  return null;
}

// Extract foreign key names from CSN ‘foreignKeys’ array
function extractForeignKeys(keys) {
  if (keys == null) return [];
  const keyArray = [];
  for (const k of keys) {
    keyArray.push(k.name);
  }
  return keyArray;
}

function extractFKFieldsFromOnCondition(on, assocName) {
  const fkFields = [];
  for (let i = 0; i < on.length; i++) {
    const cond = on[i];
    if (cond.ref && cond.ref.length === 2 && cond.ref[0] === assocName) {
      if (i + 1 < on.length && on[i + 1] === '=') {
        const fkRef = on[i + 2];
        if (fkRef?.ref) {
          const fkField = fkRef.ref[fkRef.ref.length - 1];
          if (fkField !== '$self') fkFields.push(fkField);
        }
      }
    }
  }
  return fkFields;
}

// Extract entity key fields without association keys
function extractKeys(keys) {
  return Object.keys(keys).filter((k) => keys[k].type !== 'cds.Association');
}
/**
 * Retrieves changetracking columns from entity definition
 * @param {*} entity CSN entity definition
 * @param {*} model CSN model (defaults to cds.model)
 * @param {Object|null} overrideAnnotations Optional merged annotations to use instead of entity's own
 * @returns {{ columns: Array, compositionsOfMany: Array }} Object with regular columns and composition of many info
 */
function extractTrackedColumns(entity, model = cds.context?.model ?? cds.model, overrideAnnotations = null) {
  const columns = [];
  const compositionsOfMany = [];

  for (const [name, col] of Object.entries(entity.elements)) {
    // Use override annotation if provided, otherwise use the element's own annotation
    // Normalize single-object expression annotations (compiles to {"=":"...","ref":[...]}) into array form so downstream array-based processing works uniformly
    const rawAnnotation = overrideAnnotations?.elementAnnotations?.[name] ?? col['@changelog'];
    const changelogAnnotation = rawAnnotation && typeof rawAnnotation === 'object' && !Array.isArray(rawAnnotation) ? [rawAnnotation] : rawAnnotation;

    // Skip foreign key columns (we want the generated FKs instead)
    if (col._foreignKey4 || col['@odata.foreignKey4']) continue;

    // Auto-include composition elements, unless explicitly defined with @changelog: false
    const isComposition = col.type === 'cds.Composition';
    if ((!changelogAnnotation && !isComposition) || changelogAnnotation === false) continue;

    // skip fields which might contain personal data
    if (_hasPersonalData(col)) {
      LOG.warn(`Skipped @changelog for '${name}' on entity '${entity.name}': Elements annotated with @PersonalData.IsPotentiallyPersonal or @PersonalData.IsPotentiallySensitive are not supported for change tracking.`);
      continue;
    }

    // Skip unsupported data types (Binary, LargeBinary, Vector)
    const unsupportedTypes = ['cds.LargeBinary', 'cds.Binary', 'cds.Vector'];
    if (unsupportedTypes.includes(col.type)) {
      LOG.warn(`Skipped @changelog for ${name} on entity ${entity.name}: ${col.type} change tracking not supported!`);
      continue;
    }

    const hasChildSideFK = col.type === 'cds.Composition' && (col.is2many || (col.on && col.on.some((c) => c.ref?.includes('$self'))));
    if (hasChildSideFK) {
      const compEntry = {
        name: name,
        target: col.target
      };

      // Validate @changelog on composition-of-many field
      // The annotation must only reference elements from the parent entity, not from the target entity
      if (Array.isArray(changelogAnnotation) && changelogAnnotation.length > 0) {
        const validPaths = [];
        for (const entry of changelogAnnotation) {
          if (isExpression(entry)) {
            // Expression annotation: validate that all refs within the expression reference parent entity elements
            const valid = _validateExpressionRefs(entry.xpr, name, entity);
            if (!valid) {
              LOG.warn(`Invalid @changelog expression on composition field '${name}' of entity '${entity.name}': ` + `Composition-of-many @changelog expressions must only reference elements from the parent entity. Expression ignored.`);
              continue;
            }
            if (_xprRefsPersonalData(entry.xpr, entity, model)) {
              LOG.warn(`Skipped @changelog expression on composition field '${name}' of entity '${entity.name}': Expression references a @PersonalData field.`);
              continue;
            }
            validPaths.push(entry);
            continue;
          }
          const path = entry['='];
          if (!path) continue;
          const segments = path.split('.');
          // Check: if path starts with the composition field name, it references the child
          if (segments[0] === name) {
            LOG.warn(
              `Invalid @changelog path '${path}' on composition field '${name}' of entity '${entity.name}': ` + `Composition-of-many @changelog must only reference elements from the parent entity, not from the target entity. Path ignored.`
            );
            continue;
          }
          // Validate path exists on parent entity
          if (!entity.elements?.[segments[0]]) {
            LOG.warn(`Invalid @changelog path '${path}' on composition field '${name}' of entity '${entity.name}': ` + `Element '${segments[0]}' not found on parent entity. Path ignored.`);
            continue;
          }
          if (_hasPersonalData(entity.elements[segments[0]])) {
            LOG.warn(`Skipped @changelog path '${path}' on composition field '${name}' of entity '${entity.name}': Target element is annotated with @PersonalData.`);
            continue;
          }
          validPaths.push(entry);
        }
        if (validPaths.length > 0) {
          compEntry.compositionFieldChangelog = validPaths;
        }
      }

      compositionsOfMany.push(compEntry);
      continue;
    }

    const isAssociation = col.target !== undefined; //REVISIT col.type === 'cds.Association' includes cds.common
    if (isAssociation && col.is2many && col.on) {
      LOG.debug(`Skipped @changelog for '${name}' on entity '${entity.name}': Association-to-many is not supported for change tracking.`);
      continue;
    }
    const entry = { name: name, type: col.type };
    if (col.type === 'cds.Composition' && col.is2one) entry.is2one = true;
    if (col.type === 'cds.Decimal') {
      if (col.precision != null) entry.precision = col.precision;
      if (col.scale != null) entry.scale = col.scale;
    }

    if (isAssociation) {
      // Check if association target has a table
      const targetEntity = model.definitions[col.target];
      if (!targetEntity || targetEntity['@cds.persistence.skip'] === true) {
        LOG.warn(`Skipped @changelog for ${name} on entity ${entity.name}: Association target "${col.target}" is annotated with @cds.persistence.skip!`);
        continue;
      }
      entry.target = col.target;

      // Parse on-condition mapping once for unmanaged associations
      let onMapping = null;
      if (!col.keys && col.on) {
        onMapping = [];
        for (let i = 0; i < col.on.length; i++) {
          const cond = col.on[i];
          if (cond.ref && cond.ref.length === 2 && cond.ref[0] === name) {
            const targetKey = cond.ref[1];
            if (i + 1 < col.on.length && col.on[i + 1] === '=') {
              const fkRef = col.on[i + 2];
              if (fkRef?.ref) {
                const fkField = fkRef.ref[fkRef.ref.length - 1];
                onMapping.push({ targetKey, foreignKeyField: fkField });
              }
            }
          }
        }
        if (onMapping.length === 0) onMapping = null;
      }

      // Use the resolved changelog annotation (which could be from override)
      if (Array.isArray(changelogAnnotation) && changelogAnnotation.length > 0) {
        // Check if the entry is an expression
        const expressionEntry = changelogAnnotation.find((c) => isExpression(c));
        if (expressionEntry) {
          if (_xprRefsPersonalData(expressionEntry.xpr, entity, model)) {
            LOG.warn(`Skipped @changelog expression on '${name}' of entity '${entity.name}': Expression references a @PersonalData field.`);
          } else {
            entry.altExpression = expressionEntry.xpr;
          }
        } else {
          // Path expression
          let alt = [];
          const changelogPaths = changelogAnnotation.map((c) => c['=']);
          for (const path of changelogPaths) {
            const p = validateChangelogPath(entity, path, model);
            if (!p) continue;
            if (p.includes('.')) alt.push({ path: p, source: 'assoc' });
            else alt.push({ path: p, source: 'local' });
          }
          // If alt is the same as foreign keys, no need to generate lookup
          const paths = alt.map((a) => a.path);
          if (col.keys) {
            const foreignKeys = col.keys.map((k) => k.$generatedFieldName.replaceAll('_', '.'));
            if (paths.join(',') === foreignKeys.join(',')) {
              alt = [];
            }
          } else if (onMapping) {
            // For unmanaged associations, check if all alt paths reference target keys
            const onTargetPaths = onMapping.map((m) => `${name}.${m.targetKey}`);
            if (paths.join(',') === onTargetPaths.join(',')) {
              alt = [];
            }
          }
          if (alt.length > 0) entry.alt = alt;
        }
      }

      if (col.keys) {
        // for managed associations with generated foreign keys
        entry.foreignKeys = col.keys.flatMap((k) => k.ref);
      } else if (onMapping) {
        // for unmanaged associations, use the parsed on-condition mapping
        entry.on = onMapping;
      }
    } else if (Array.isArray(changelogAnnotation) && changelogAnnotation.length > 0) {
      // Non-association elements with expression-based @changelog labels
      const expressionEntry = changelogAnnotation.find((c) => isExpression(c));
      if (expressionEntry) {
        if (_xprRefsPersonalData(expressionEntry.xpr, entity, model)) {
          LOG.warn(`Skipped @changelog expression on '${name}' of entity '${entity.name}': Expression references a @PersonalData field.`);
        } else {
          entry.altExpression = expressionEntry.xpr;
        }
      }
    }
    columns.push(entry);
  }
  return { columns, compositionsOfMany };
}

function getObjectIDs(entity, model = cds.context?.model ?? cds.model, overrideEntityAnnotation = null) {
  if (!entity) return [];
  // Use override annotation if provided, otherwise use the entity's own annotation
  // Normalize single-object expression annotations (from @changelog: (expr) syntax) into array form
  const rawEntityAnnotation = overrideEntityAnnotation ?? entity['@changelog'];
  if (!rawEntityAnnotation || rawEntityAnnotation === true) return [];
  const entityAnnotation = rawEntityAnnotation && typeof rawEntityAnnotation === 'object' && !Array.isArray(rawEntityAnnotation) ? [rawEntityAnnotation] : rawEntityAnnotation;
  const ids = [];

  for (const entry of entityAnnotation) {
    // Check if this is a CDS expression (e.g., @changelog: [(firstName || ' ' || lastName)])
    if (isExpression(entry)) {
      if (_xprRefsPersonalData(entry.xpr, entity, model)) {
        LOG.warn(`Skipped @changelog objectID expression on entity '${entity.name}': Expression references a @PersonalData field.`);
        continue;
      }
      ids.push({ name: entry['='], expression: entry.xpr, included: false });
      continue;
    }

    const field = entry['='];
    if (!field) {
      LOG.debug(`Skipped @changelog objectID entry on entity '${entity.name}': No path value found in annotation entry.`);
      continue;
    }

    // Validate and normalize the @changelog path
    const normalized = validateChangelogPath(entity, field, model);
    if (!normalized) continue;

    // Check if the field is directly included or needs to be computed
    const element = entity.elements?.[normalized];
    const included = !!element && !element['@Core.Computed'];
    ids.push({ name: normalized, included });
  }
  return ids;
}

// Join helper for association lookups
// "a || ', ' || b || ', ' || c"
function buildConcatXpr(columns) {
  const parts = [];
  for (let i = 0; i < columns.length; i++) {
    const ref = { ref: columns[i].split('.') };
    parts.push(ref);
    if (i < columns.length - 1) {
      parts.push('||');
      parts.push({ val: ', ' });
      parts.push('||');
    }
  }
  return { xpr: parts, as: 'value' };
}

/**
 * Checks if an array looks like a CDS ref (array of only plain strings).
 * Distinguishes from xpr arrays which contain mixed token types.
 */
function _isRefArray(arr) {
  return Array.isArray(arr) && arr.length > 0 && arr.every((s) => typeof s === 'string');
}

/**
 * Builds a CQN expression column for use in a SELECT query.
 * @param {Array} expression A single CDS expression (xpr array or ref array)
 * @returns {Object} CQN column object with xpr and alias
 */
function buildExpressionColumn(expression) {
  if (_isRefArray(expression)) {
    return { ref: expression, as: 'value' };
  }
  return { xpr: expression, as: 'value' };
}

const transformName = (name) => {
  const quoted = cds.env?.sql?.names === 'quoted';
  return quoted ? `"${name}"` : name.replace(/\./g, '_').toUpperCase();
};

/**
 * Gets the FK binding from a composition target entity back to its parent/root entity.
 * Used for composition of many tracking where we need to find the parent's key from the child.
 * @param {*} targetEntity The composition target entity (e.g., Books)
 * @param {*} rootEntity The root/parent entity (e.g., BookStores)
 * @returns {Array|null} Array of FK field names on target that point to root, or null if not found
 */
function getCompositionParentBinding(targetEntity, rootEntity) {
  // Look for association/backlink from target to root
  for (const element of targetEntity.elements) {
    if ((element.type === 'cds.Association' || element.type === 'cds.Composition') && element.target === rootEntity.name) {
      // managed association: use foreignKeys
      const fks = extractForeignKeys(element.foreignKeys);
      if (fks.length > 0) {
        return fks.map((fk) => `${element.name}_${fk}`);
      }
      // unmanaged association: extract FK fields from on condition
      if (element.on) {
        const fkFields = extractFKFieldsFromOnCondition(element.on, element.name);
        if (fkFields.length > 0) return fkFields;
      }
      // fallback: if no explicit foreignKeys, CAP names them as <assocName>_<rootKey>
      const rootKeys = extractKeys(rootEntity.keys);
      if (rootKeys.length > 0) {
        return rootKeys.map((k) => `${element.name}_${k}`);
      }
    }
  }

  // Check for up_ link (inline compositions)
  const upElement = targetEntity.elements?.up_;
  if (upElement?.type === 'cds.Association') {
    const fks = extractForeignKeys(upElement.foreignKeys);
    if (fks.length > 0) return fks.map((fk) => `up__${fk}`);
  }

  // Handle composition of one: root has FK to child (reverse lookup needed)
  for (const element of rootEntity.elements) {
    if (element.type === 'cds.Composition' && element.target === targetEntity.name && element.is2one) {
      // Get child entity keys for the reverse lookup
      const childKeys = extractKeys(targetEntity.keys);
      if (childKeys.length > 0) {
        return {
          type: 'compositionOfOne',
          compositionName: element.name,
          childKeys: childKeys,
          rootEntityName: rootEntity.name
        };
      }
    }
  }

  return null;
}

function getLocalizedLookupInfo(targetEntityName, altFields, model = cds.context?.model ?? cds.model) {
  if (!altFields || altFields.length === 0) return null;
  const targetEntity = model.definitions[targetEntityName];
  if (!targetEntity) return null;

  // Check if target entity has localized data (i.e., a .texts table exists)
  const textsEntityName = `${targetEntityName}.texts`;
  const textsEntity = model.definitions[textsEntityName];
  if (!textsEntity) return null;

  // Get the key fields of the target entity (needed for the texts table join)
  const targetKeys = extractKeys(targetEntity.keys);
  if (targetKeys.length === 0) return null;

  // Check which alt fields are localized on the target entity
  // alt fields are in format "assocName.field" - we need to check if "field" is localized
  const localizedFields = [];
  for (const altField of altFields) {
    const fieldName = altField.split('.').slice(1).join('.');
    if (!fieldName) continue;

    if (textsEntity.elements?.[fieldName]) {
      localizedFields.push(fieldName);
    }
  }

  if (localizedFields.length === 0) return null;

  return {
    textsEntity: textsEntityName,
    localizedFields,
    keys: targetKeys
  };
}

module.exports = {
  isExpression,
  collectRefsFromXpr,
  validateExpressionRefs: _validateExpressionRefs,
  extractForeignKeys,
  extractKeys,
  extractTrackedColumns,
  getObjectIDs,
  buildConcatXpr,
  buildExpressionColumn,
  transformName,
  getCompositionParentBinding,
  getLocalizedLookupInfo
};
