		...
		if (isAssociation) {
			...
			if (Array.isArray(changelogAnnotation) && changelogAnnotation.length > 0) {
				...
				for (const path of changelogPaths) {
					...
					if (!p) continue;
					// Distinguish between paths referencing the association target and paths referencing the entity itself
					if (p.includes('.')) alt.push(p);
					else altLocal.push(p);
				}
				if (alt.length > 0) entry.alt = alt;
				if (altLocal.length > 0) entry.altLocal = altLocal;
			}
