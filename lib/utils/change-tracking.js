...
function extractTrackedColumns(entity, model = cds.model) {
    ...
    for (const [name, col] of Object.entries(entity.elements)) {
        ...
        if (!col['@changelog'] || col._foreignKey4 || col["@odata.foreignKey4"]) continue;
        ...
        if (col.type === 'cds.Composition' && col.is2many) {
            LOG.warn(`Skipped @changelog for ${name} on entity ${entity.name}: Compositions of many not supported!`);
            continue;
        };
