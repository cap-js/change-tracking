 ...
 */
function writeLabelsCSV(entities, model, dir = 'db/src/gen/data') {
...
function ensureUndeployJsonHasTriggerPattern() {
  // Only applicable during cds build from the project root (skip sidecar)
  if (!fs.existsSync(cds.utils.path.join(cds.root, 'db'))) return;

  const undeployPath = cds.utils.path.join(cds.root, 'db', 'undeploy.json');
