 ...
 */
function parseCompositionFieldChangelog(compositionFieldChangelog, parentEntity, parentKeyBinding, refRow) {
	...
	if (expressionEntry) {
		...
		for (let i = 0; i < parentKeys.length; i++) {
			where[parentKeys[i]] = { val: `${refRow}.${parentKeyBinding[i]}`, literal: 'sql' };
		}
