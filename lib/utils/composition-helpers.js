...
function getCompositionParentInfo(entity, parentEntity, parentMergedAnnotations) {
	...
	for (const [elemName, elem] of Object.entries(parentEntity.elements)) {
		...
		const changelogAnnotation = parentMergedAnnotations?.elementAnnotations?.[elemName] ?? elem['@changelog'];
			...
			if (Array.isArray(normalizedAnnotation)) {
				...
				for (const entry of normalizedAnnotation) {
					if (entry && typeof entry === 'object' && entry.xpr) {
						...
						const valid = utils.validateExpressionRefs(entry.xpr, elemName, parentEntity);
						if (valid) {
 ...
 * @returns {null | { levels: Array, isCompositionOfOne: boolean, compositionFieldChangelog: Array|null, parentKeyBinding: Array|Object }}
 */
function getCompositionHierarchy(entity, parentEntity, parentMergedAnnotations, ancestorChain, model) {
	...
	const { parentEntityName, compositionFieldName, parentKeyBinding, isCompositionOfOne, compositionFieldChangelog } = parentInfo;

	// Build the levels array: immediate parent + ancestors
	const levels = [{ entityName: parentEntityName, compositionFieldName, keyBinding: parentKeyBinding }];
	...
	return { levels, isCompositionOfOne, compositionFieldChangelog, parentKeyBinding };
}

module.exports = { getCompositionParentInfo, getGrandParentCompositionInfo, getAncestorCompositionChain, getCompositionHierarchy, resolveCompositionObjectIDs, parseCompositionFieldChangelog };
