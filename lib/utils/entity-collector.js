...
function getEntitiesForTriggerGeneration(model, collected) {
	...
	// Track the depth of each entity from its nearest tracked root ancestor
	const entityDepth = new Map();
	for (const { dbEntityName } of result) entityDepth.set(dbEntityName, 0);
	...
	for (let depth = 1; depth < maxDepth; depth++) {
		...
		for (const { dbEntityName, mergedAnnotations } of currentEntities) {
			...
			for (const element of Object.values(dbEntity.elements)) {
				...
				// Skip if composition field is explicitly set to false 
				...
				// Only auto-track if target entity has at least one @changelog element
				...
				entityDepth.set(element.target, depth);
				newEntities.push({ dbEntityName: element.target, mergedAnnotations: null });
