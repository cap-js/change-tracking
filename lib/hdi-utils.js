...
function _getObjectID(entity) {
	...
	for (const { ['=']: field } of entity['@changelog']) {
		...
		for (let i = 0; i < segments.length - 1; i++) {
			...
			if (!assoc?.isAssociation || (assoc.isComposition && assoc.is2many)) {
				broken = true;
				break;
			}
...
const getInsertChangesSQL = (attribute, oldVal, newVal) => {
	return insertChangesSQL.replace('{{attribute}}', `'${attribute}'`).replace('{{valueChangedFrom}}', oldVal).replace('{{valueChangedTo}}', newVal);
};
	...
	const selects = objectIDs
		.map((oi) => {
			...
			const query = SELECT.one
				.from(entityName)
				.columns(`{ ${oi.name.replaceAll('_', '.')} }`)
				.where(
					entityKeys.reduce((acc, k) => {
						acc[k] = { val: `:${refRow}.${k}` };
						return acc;
					}, {})
				);
...
function _generateUpdateTrigger(tableName, columns, triggerDeclaration, objectIDSQLInit) {
	const changesCreation = columns
      ...
      ${handleAssociationLookUp(c, 'old')}`
				: '';
			const condition = assocLookUp ? c.foreignKeys.map((k) => _nullSafeChanged(`${c.name}_${k}`)).join(' OR ') : _nullSafeChanged(c.name);
