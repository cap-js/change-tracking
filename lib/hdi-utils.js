...
function _callChangesProcedure(field, oldVal, newVal, operation) {
  return `
    CALL CREATE_CHANGES(
      :v_change_id,
      '${field}',
...
function _generateUpdateTrigger(name, tableName, dbName, columns, keys) {
  const changesCreation = columns.map(c => {
    ...
    ${assocLookUp}\n
    ${_callChangesProcedure(c.attribute, oldVal, newVal, 'update')}
    END IF;`;
  }).join('\n');
...
function _generateCreateTrigger(name, tableName, dbName, columns, keys) {
  const changesCreation = columns.map(c => {
    ...
    const x = _callChangesProcedure(c.attribute, 'NULL', newVal, 'create');

    return `${assocLookUp}\n${_callChangesProcedure(c.attribute, 'NULL', newVal, 'create')}`;
  }).join('\n');
...
function _generateDeleteTriggerPreserve(name, tableName, dbName, columns, keys) {
  const changesCreation = columns.map(c => {
    ...
    return `${assocLookUp}\n${_callChangesProcedure(c.attribute, oldVal, 'NULL', 'delete')}`;
  }).join('\n');
