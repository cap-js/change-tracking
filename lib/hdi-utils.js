...
function generateTriggersForEntity(name, def) {
  const triggers = [];
  const tableName = _getTableName(def); // REVISIT: cds.db not initialized during compile (cds.db.resolve.table(def))
...
function _createTriggerDeclaration(name, tableName, rowRef, keys) {
  return `
...
function _extractTrackedColumns(elements) {
  ...
  for (const col of elements) {
    ...
    const name = isAssociation ? `${col.name}_${col.keys?.[0]?.ref?.[0]}` : col.name;
...
function _generateUpdateTrigger(name, tableName, dbName, columns, keys) {
  const changesCreation = columns.map(c => {
    const assocLookUp = c.target && c.alt
      ? `DECLARE v_newAttr NVARCHAR(5000); DECLARE v_oldAttr NVARCHAR(5000);
           SELECT ${c.alt} INTO v_newAttr FROM ${_transformedName(c.target)} WHERE ID = :new.${c.name};
           SELECT ${c.alt} INTO v_oldAttr FROM ${_transformedName(c.target)} WHERE ID = :old.${c.name};`
      : '';
    return `IF: old.${c.name} <> :new.${c.name} THEN
      ${assocLookUp}
      CALL CREATE_CHANGES(
        :v_change_id,
        '${c.attribute}',
        ${assocLookUp ? ':v_oldAttr' : ':old.' + c.name},
        ${assocLookUp ? ':v_newAttr' : ':new.' + c.name},
        :v_entityID,
        :v_entity,
        :v_serviceEntity,
        :v_parentEntityID,
  ...
  const ofClause = columns.length > 0 ? `OF ${columns.map(c => c.name).join(', ')} ` : '';

  return {
    name: name + '_CT_UPDATE',
    sql: `TRIGGER CT_UPDATE_${dbName} AFTER UPDATE ${ofClause}ON ${dbName}
    REFERENCING NEW ROW new, OLD ROW old
    FOR EACH ROW
    BEGIN
      ${_createTriggerDeclaration(name, tableName, 'old', keys)}
...
function _generateCreateTrigger(name, tableName, dbName, columns, keys) {
  const changesCreation = columns.map(c => {
    const assocLookUp = c.target && c.alt
      ? `SELECT ${c.alt} INTO v_new_${c.name} FROM ${_transformedName(c.target)} WHERE ID = :new.${c.name};`
      : '';
    return `${assocLookUp}
      CALL CREATE_CHANGES(
        :v_change_id,
        '${c.attribute}',
        NULL,
        ${assocLookUp ? `:v_new_${c.name}` : ':new.' + c.name},
        :v_entityID,
        :v_entity,
        :v_serviceEntity,
        :v_parentEntityID,
  ...
  return {
    ...
    sql: `TRIGGER CT_CREATE_${dbName} AFTER INSERT ON ${dbName}
    REFERENCING NEW ROW new
    FOR EACH ROW
    BEGIN
      ${_createTriggerDeclaration(name, tableName, 'new', keys)}
...
function _generateDeleteTriggerPreserve(name, tableName, dbName, columns, keys) {
  const changesCreation = columns.map(c => {
    const assocLookUp = c.target && c.alt
      ? `SELECT ${c.alt} INTO v_old_${c.name} FROM ${_transformedName(c.target)} WHERE ID = :old.${c.name};`
      : '';
    return `${assocLookUp}
      CALL CREATE_CHANGES(
        :v_change_id,
        '${c.attribute}',
        ${assocLookUp ? `:v_old_${c.name}` : ':old.' + c.name},
        NULL,
        :v_entityID,
        :v_entity,
        :v_serviceEntity,
  ...
  return {
    ...
    sql: `TRIGGER CT_DELETE_${dbName} AFTER DELETE ON ${dbName}
