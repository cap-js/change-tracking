...
function buildObjectIDExpr(objectIDs, entity, refRow, model) {
  ...
  for (const oid of objectIDs) {
    if (oid.included) {
      ...
      parts.push(`COALESCE(TO_NVARCHAR(:${refRow}.${quote(oid.name)}), '<empty>')`);
      nullChecks.push(`:${refRow}.${quote(oid.name)} IS NULL`);
    } else if (oid.expression) {
      // Expression-based ObjectID: inline expression using trigger row refs
      const CQN2SQLClass = require('@cap-js/hana').CQN2SQL;
      const sql = buildExpressionSQL(oid.expression, entity, refRow, model, toSQL, (r, c) => `:${r}.${quote(c)}`, CQN2SQLClass);
...
function buildTriggerContext(entity, objectIDs, refRow, model) {
  const keys = utils.extractKeys(entity.keys);
  return {
    entityKey: entityKeyExpr(keys.map((k) => `:${refRow}.${quote(k)}`)),
    objectID: buildObjectIDExpr(objectIDs, entity, refRow, model)
  };
}
