...
function buildAssocLookup(column, refRow, entityKey, model) {
	let where = {};
		...
		if (localizedInfo) {
			...
			parts.push(`COALESCE((${toSQL(textsQuery, model)}), (${toSQL(baseQuery, model)}))`);
		} else {
			const query = SELECT.one.from(col.target).columns(columns).where(where);
			parts.push(`(${toSQL(query, model)})`);
		}
...
function getLabelExpr(col, refRow, model) {
	...
	if (col.target && col.alt) {
		parts.push(buildAssocLookup(col, refRow, model));
	}

	if (col.altLocal) {
		for (const field of col.altLocal) parts.push(`TO_NVARCHAR(:${refRow}.${field})`);
	}
