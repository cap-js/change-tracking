...
function _getHanaCQN2SQLClass() {
	return require('@cap-js/hana').CQN2SQL;
}

function buildExpressionSubquery(entityName, exprColumn, keys, refRow, model) {
...
function getLabelExpr(col, refRow, model, entityName = null) {
	...
	if (col.altExpressions && col.altExpressions.length > 0 && entityName) {
		const CQN2SQLClass = _getHanaCQN2SQLClass();
		const sqlParts = col.altExpressions.map((xpr) =>
			// Cast to NVARCHAR to ensure UNION compatibility — expression results may be numeric/date/etc.
			`TO_NVARCHAR(${buildExpressionSQL(xpr, entityName, refRow, model, CQN2SQLClass, toSQL, _hanaRefFormat)})`
		);
		return sqlParts.length === 1 ? sqlParts[0] : sqlParts.join(" || ', ' || ");
	}
...
function buildObjectIDExpr(objectIDs, entity, rowRef, model) {
	...
	for (const oid of objectIDs) {
		if (oid.included) {
			...
			const sql = buildExpressionSubquery(entity.name, exprColumn, keys, rowRef, model);
