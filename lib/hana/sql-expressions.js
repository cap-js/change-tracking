...
function buildAssocLookup(col, assocPaths, refRow, model) {
	...
	if (col.foreignKeys) {
		where = col.foreignKeys.reduce((acc, k) => {
			acc[k] = { val: `${refRow}.${col.name}_${k}`, literal: 'sql' };
			return acc;
		}, {});
	} else if (col.on) {
		where = col.on.reduce((acc, mapping) => {
			acc[mapping.targetKey] = { val: `${refRow}.${mapping.foreignKeyField}`, literal: 'sql' };
			return acc;
		}, {});
	}
...
function buildObjectIDExpr(objectIDs, entity, rowRef, model) {
	...
	const entityKey = entityKeyExpr(keys.map((k) => `${rowRef}.${k}`));
	...
	for (const oid of objectIDs) {
		if (oid.included) {
			...
			const where = keys.reduce((acc, k) => {
				acc[k] = { val: `${rowRef}.${k}`, literal: 'sql' };
				return acc;
			}, {});
	...
	return `COALESCE(NULLIF(LTRIM(${concatExpr}, ', '), ''), ${entityKey})`;
}
