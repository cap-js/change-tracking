...
function buildObjectIDExpr(objectIDs, entity, rowRef, model) {
	...
	for (const oid of objectIDs) {
		if (oid.included) {
			// Included fields are directly available from the trigger row — never NULL for objectID purposes
	...
	// First part is always included; subsequent parts are prefixed with ', ' only if non-NULL
	if (parts.length === 1) {
		return `COALESCE(${parts[0]}, ${entityKeyExpr})`;
	}
	const [first, ...rest] = parts;
	const concatExpr = `${first}` + rest.map((p) => ` || CASE WHEN ${p} IS NOT NULL THEN ', ' || ${p} ELSE '' END`).join('');
	return `COALESCE(NULLIF(${concatExpr}, ''), ${entityKeyExpr})`;
}
