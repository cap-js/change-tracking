...
function registerHDICompilerHook() {
	...
	cds.compiler.to.hdi.migration = function (csn, options, beforeImage) {
		...
		// Generate restore backlinks procedure unless disabled via feature flag
		const config = cds.env.requires?.['change-tracking'];
		if (!config?.disableRestoreBacklinks) {
			const { generateRestoreBacklinksProcedure } = require('./procedure.js');
