...
function registerHDICompilerHook() {
	...
	// When addMigrationTable is enabled, replace the .hdbtable SQL for Changes with the custom migration table
	const _compile_to_hana = cds.compile.to.hana;
	cds.compile.to.hana = function (csn, o, beforeCsn) {
		const config = cds.env.requires?.['change-tracking'];
		const result = _compile_to_hana.call(this, csn, o, beforeCsn);
		if (!config?.addMigrationTable) return result;

		const { getMigrationTableSQL } = require('./migrationTable.js');
		return (function* () {
			for (const [content, meta] of result) {
				if (meta.file === 'sap.changelog.Changes.hdbtable') {
					yield [getMigrationTableSQL(), meta];
				} else {
					yield [content, meta];
				}
			}
