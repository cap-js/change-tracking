...
SET entityKey =
    CASE
        ...
        WHEN OCCURRENCES_REGEXPR(', ' IN entityKey) = 1 THEN
            HIERARCHY_COMPOSITE_ID(
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 1 GROUP 1)
                ),
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 2 GROUP 1)
                )
            )

        WHEN OCCURRENCES_REGEXPR(', ' IN entityKey) = 2 THEN
            HIERARCHY_COMPOSITE_ID(
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 1 GROUP 1)
                ),
                ...
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 3 GROUP 1)
                )
            )

        WHEN OCCURRENCES_REGEXPR(', ' IN entityKey) = 3 THEN
            HIERARCHY_COMPOSITE_ID(
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 1 GROUP 1)
                ),
                ...
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 4 GROUP 1)
                )
            )

        WHEN OCCURRENCES_REGEXPR(', ' IN entityKey) = 4 THEN
            HIERARCHY_COMPOSITE_ID(
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 1 GROUP 1)
                ),
                ...
                TRIM(
                    SUBSTR_REGEXPR('[^=,]+=[[:space:]]*([^,]+)' IN entityKey OCCURRENCE 5 GROUP 1)
                )
            )
...
UPDATE SAP_CHANGELOG_CHANGES c
  SET transactionID = (
    SELECT txn FROM (
      SELECT changeLog_ID, DENSE_RANK() OVER (ORDER BY changeLog_ID) AS txn
      FROM SAP_CHANGELOG_CHANGES
      WHERE changeLog_ID IS NOT NULL
      GROUP BY changeLog_ID
    ) m
    WHERE m.changeLog_ID = c.changeLog_ID
  )
  WHERE c.changeLog_ID IS NOT NULL;
