...
const { entityKeyExpr } = require('./sql-expressions.js');
...
function _buildChildKeyJoinCondition(childKeys, alias, changesAlias) {
	const entityKey = entityKeyExpr(childKeys.map((k) => `${alias}.${k}`));

	// Single key: straightforward join
	if (childKeys.length <= 1) {
		return `${changesAlias}.ENTITYKEY = ${entityKey}`;
	}
	...
	return `(${changesAlias}.ENTITYKEY = ${entityKey} OR ${changesAlias}.ENTITYKEY = TO_NVARCHAR(${alias}.${lastKey}))`;
}
...
function _generateCompositionBlock(comp) {
	...
	const parentKeyFromChild = entityKeyExpr(fkFields.map((fk) => `child_data.${fk}`));
	...
	if (simpleObjectIDs.length > 0) {
		const parentKeyWhereClause = `${entityKeyExpr(parentKeys)} = grp.PARENT_ENTITYKEY`;
		const parts = simpleObjectIDs.map((oid) => `COALESCE(TO_NVARCHAR((SELECT ${oid.name} FROM ${parentTableName} WHERE ${parentKeyWhereClause})), '')`);
	...
	const childEntityKeyExpr = entityKeyExpr(childKeys.map((k) => `child_data.${k}`));
	...
	if (grandParentCompositionInfo) {
		...
		const grandParentKeyFromParent = entityKeyExpr(grandParentKeyBinding.map((fk) => `parent_data.${fk}`));
		...
		if (simpleGPObjectIDs.length > 0) {
			const gpKeyWhereClause = `${entityKeyExpr(grandParentKeys)} = grp2.GP_ENTITYKEY`;
			const gpParts = simpleGPObjectIDs.map((oid) => `COALESCE(TO_NVARCHAR((SELECT ${oid.name} FROM ${grandParentTableName} WHERE ${gpKeyWhereClause})), '')`);
	...
	FROM (
		...
		INNER JOIN ${parentTableName} parent_data
			ON comp2.ENTITYKEY = ${entityKeyExpr(parentKeys.map((pk) => `parent_data.${pk}`))}
