 ...
 * Produces: COALESCE(NULLIF(COALESCE(cast(col1 as String),'<empty>') || ', ' || ..., ''), cast(ID as String))
 */
function _buildObjectIdCQN(objectIDs, entityKeys, alias) {
	...
	return {
		func: 'coalesce',
		args: [{ func: 'nullif', args: [concatXpr, { val: '' }] }, fallbackKeyCQN]
	};
}

/**
 * Builds a CQN expression for a parent/grandparent objectID using scalar subqueries.
 * Produces: COALESCE(NULLIF(COALESCE((SELECT col FROM entity WHERE key = ref), '') || ..., ''), keyRef)
...
function _buildParentObjectIdCQN(objectIDs, entityName, entityKeys, keyRefSQL) {
	...
	const parts = objectIDs.map((oid) => {
		const query = SELECT.from(entityName).columns(oid.name).where(where);
		return { func: 'coalesce', args: [{ xpr: [query], cast: { type: 'cds.String' } }, { val: '' }] };
	});
	...
	return {
		func: 'coalesce',
		args: [{ func: 'nullif', args: [concatXpr, { val: '' }] }, { val: keyRefSQL, literal: 'sql' }]
	};
}
