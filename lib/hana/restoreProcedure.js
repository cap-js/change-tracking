...
function _buildChildKeyJoinCQN(childKeys, childAlias, changesAlias) {
  const castKey = childKeys.length <= 1 ? { ref: [childAlias, childKeys[0]], cast: { type: 'cds.String' } } : { func: 'HIERARCHY_COMPOSITE_ID', args: childKeys.map((k) => ({ ref: [childAlias, k] })) };
...
function _buildParentObjectIdCQN(objectIDs, entityName, entityKeys, keyRefSQL) {
  ...
  if (entityKeys.length <= 1) {
    ...
    where = [{ func: 'HIERARCHY_COMPOSITE_ID', args: entityKeys.map((k) => ({ ref: [k] })) }, '=', { val: keyRefSQL, literal: 'sql' }];
  }
...
function _generateCompositionBlock(comp, model) {
  const { childEntityName, parentEntityName, compositionField, fkFields, childKeys, parentKeys, rootObjectIDs, childObjectIDs, grandParentCompositionInfo, grandParentKeys, grandParentObjectIDs } = comp;
  ...
  const step1InnerFrom = {
    join: 'inner',
    args: [
      { ref: [changesEntity], as: 'c' },
      { ref: [childEntityName], as: 'child_data' }
    ],
        ...
        'exists',
        {
          SELECT: {
            ...
            where: [
              { ref: ['p', 'entity'] },
              '=',
              { val: parentEntityName },
              'and',
              { ref: ['p', 'attribute'] },
              '=',
              { val: compositionField },
              'and',
              { ref: ['p', 'valueDataType'] },
              '=',
              { val: 'cds.Composition' },
  ...
  const simpleObjectIDs = rootObjectIDs?.filter((oid) => !oid.expression && !oid.name?.includes('.')) ?? [];
  ...
  const childObjectIdCQN = _buildObjectIdCQN(childObjectIDs?.filter((oid) => !oid.expression && !oid.name?.includes('.')) ?? [], childKeys, 'child_data');
  ...
  const step2From = {
    ...
    on: [
      ...
      { ref: ['p', 'transactionID'] },
      '=',
      { ref: ['c2', 'transactionID'] },
      'and',
      { ref: ['p', 'entityKey'] },
      '=',
      _buildEntityKeyCQN(fkFields, 'child_data')
    ]
  };
  ...
  if (grandParentCompositionInfo) {
    ...
    const simpleGPObjectIDs = grandParentObjectIDs?.filter((oid) => !oid.expression && !oid.name?.includes('.')) ?? [];
    ...
    const step3aQuery = {
      SELECT: {
        from: {
          join: 'inner',
          args: [
            { ref: [changesEntity], as: 'comp2' },
            { ref: [parentEntityName], as: 'parent_data' }
          ],
          ...
          'exists',
          {
            SELECT: {
              ...
              where: [
                { ref: ['gp', 'entity'] },
                '=',
                { val: grandParentEntityName },
                'and',
                { ref: ['gp', 'attribute'] },
                '=',
                { val: grandParentCompositionFieldName },
                'and',
                { ref: ['gp', 'valueDataType'] },
                '=',
                { val: 'cds.Composition' },
                'and',
                { ref: ['gp', 'transactionID'] },
                '=',
                { ref: ['comp2', 'transactionID'] },
                'and',
                { ref: ['gp', 'entityKey'] },
                '=',
                gpKeyFromParentCQN
