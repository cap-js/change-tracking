...
function buildCompOfManyRootObjectIDSelect(rootEntity, rootObjectIDs, binding, refRow, model, keyValueExprs = null) {
	...
	// Build concatenation that skips NULL parts (e.g., unset association paths)
	if (parts.length === 1) {
		return `COALESCE(${parts[0]}, ${rootEntityKeyExpr})`;
	}
	const [first, ...rest] = parts;
	const concatExpr = `${first}` + rest.map((p) => ` || CASE WHEN ${p} IS NOT NULL THEN ', ' || ${p} ELSE '' END`).join('');
	return `COALESCE(NULLIF(${concatExpr}, ''), ${rootEntityKeyExpr})`;
}
...
function buildCompositionOfOneParentContext(compositionParentInfo, rootObjectIDs, modification, rowRef, model, childObjectIDExpr = null) {
	...
	// since objectID now reflects the child entity (may differ per child row).
...
function buildCompositionParentContext(compositionParentInfo, rootObjectIDs, modification, rowRef, model, ancestorCompositionChain = [], childObjectIDExpr = null) {
	const { parentEntityName, compositionFieldName, parentKeyBinding } = compositionParentInfo;
	...
	const immediateParentObjectIDExpr = childObjectIDExpr ?? buildCompOfManyRootObjectIDSelect(rootEntity, rootObjectIDs, parentKeyBinding, rowRef, model);
