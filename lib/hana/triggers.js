...
function buildTriggerContext(entity, objectIDs, rowRef, model, compositionHierarchy = null) {
  ...
  return {
    ...
    parentLookupExpr: compositionHierarchy !== null ? 'parent_id' : null
  };
}
...
function generateCreateTrigger(entity, columns, objectIDs, parentObjectIDs, model, compositionHierarchy = null) {
  const ctx = buildTriggerContext(entity, objectIDs, 'new', model, compositionHierarchy);
  ...
  const compositionParentContext = compositionHierarchy
    ? buildCompositionParentContext(compositionHierarchy, parentObjectIDs, 'new', model, buildObjectIDExpr(objectIDs, entity, 'new', model), (changelog, parentName, parentEntity, keyBinding) =>
        buildCompositionFieldObjectID(changelog, parentEntity, keyBinding, 'new', model)
      )
    : null;
...
function generateUpdateTrigger(entity, columns, objectIDs, parentObjectIDs, model, compositionHierarchy = null) {
  const ctx = buildTriggerContext(entity, objectIDs, 'new', model, compositionHierarchy);
...
function generateDeleteTriggerPreserve(entity, columns, objectIDs, parentObjectIDs, model, compositionHierarchy = null) {
  const ctx = buildTriggerContext(entity, objectIDs, 'old', model, compositionHierarchy);

  // Build context for composition parent entry if this is a tracked composition target
  const compositionParentContext = compositionHierarchy
    ? buildCompositionParentContext(compositionHierarchy, parentObjectIDs, 'old', model, buildObjectIDExpr(objectIDs, entity, 'old', model), (changelog, parentName, parentEntity, keyBinding) =>
        buildCompositionFieldObjectID(changelog, parentEntity, keyBinding, 'old', model)
      )
    : null;
...
function generateDeleteTrigger(entity, columns, objectIDs, parentObjectIDs, model, compositionHierarchy = null) {
  const keys = utils.extractKeys(entity.keys);
  const ctx = buildTriggerContext(entity, objectIDs, 'old', model, compositionHierarchy);
...
function generateHANATriggers(csn, entity, parentEntity = null, mergedAnnotations = null, parentMergedAnnotations = null, grandParentContext = {}) {
  ...
  if (!config?.disableCreateTracking) {
    triggers.push(generateCreateTrigger(entity, trackedColumns, objectIDs, parentObjectIDs, csn, compositionHierarchy));
  }
  if (!config?.disableUpdateTracking) {
    triggers.push(generateUpdateTrigger(entity, trackedColumns, objectIDs, parentObjectIDs, csn, compositionHierarchy));
  }
  if (!config?.disableDeleteTracking) {
    ...
    triggers.push(generateDeleteTriggerFunc(entity, trackedColumns, objectIDs, parentObjectIDs, csn, compositionHierarchy));
  }
