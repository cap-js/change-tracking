...
const getCurObjFromDbQuery = async function (entityName, queryVal, /**optional*/ queryKey='ID', locale) {
  if (!queryVal) return {}
  let obj = {};
  if (locale) {
    obj = await SELECT.one.from(entityName).where({[queryKey]: queryVal, locale});
  } else {
    obj = await SELECT.one.from(entityName).where({[queryKey]: queryVal});
  }
  // REVISIT: This always reads all elements -> should read required ones only!
  // const obj = await SELECT.one.from(entityName).where({[queryKey]: queryVal, locale})
  return obj || {}
}
...
async function getObjectId (reqData, entityName, fields, curObj) {
  ...
  for (let field of fields) {
    ...
    if (path.length > 1) {
      ...
      let _db_data = {};
      if (e.localized && entity.texts?.name) {
        let data = JSON.stringify(req_data) === '{}' ? db_data : req_data;
        let keysObj = getKey(entity, data);
        // _db_data = await getCurObjFromDbQuery(entity.texts.name, req_data['code'] || db_data['code'], 'code', reqData.locale);
        // _db_data = await SELECT.one.from(entity.texts.name).where({'code': db_data['code'], 'name': db_data['name'], locale: reqData.locale || 'en'});
        _db_data = await SELECT.one.from(entity.texts.name).where({...keysObj, locale: reqData.locale || 'en'}) || {};
      }
      let obj = _db_data[field] || (req_data[field] || db_data[field])
      if (obj) all.push(obj)
    }
  }
  return all.join(', ')
