...
function _getObjectIDSelect(objectIDs, entityName, entityKeys, refRow) {
    ...
    for (const objectID of objectIDs) {
        ...
        const query = SELECT.one.from(entityName).columns(`{ ${objectID.name.replaceAll('_', '.')}}`).where(where);
        objectID.selectSQL = _toSQLite(query);
    }

    const objectID = objectIDs.map(id => id.included
    ...
    for (const col of entity.elements) {
        ...
        if (isAssociation) {
            ...
            if (col['@changelog'].length > 0) {
                entry.alt = col['@changelog'].map(c => c['='].replace(col.name + '.', ''));;
            }
