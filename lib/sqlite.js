...
function _getObjectIDSelect(objectIDs, entityName, entityKeys, refRow) {
	...
	for (const objectID of objectIDs) {
		...
		const query = SELECT.one
			.from(entityName)
			.columns(`{ ${objectID.name.replaceAll('_', '.')}}`)
			.where(where);
...
function validateChangelogPath(entity, path) {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (!element) {
			...
			if (currentEntity.elements) {
				...
				if (currentEntity.elements[flattened]) {
					if (walked.length < 2) return flattened;
					let normalizedPath = walked.slice(1).join('.');
					return `${normalizedPath}.${flattened}`;
				}
				return null;
			}
		...
		if (i === segments.length - 1) {
			const normalizedPath = walked.slice(1);
	...
	const entriesSQL = columns
		.map((col) => {
			...
			if (col.target && col.alt) {
				...
				const columns = col.alt.length === 1 ? col.alt[0] : buildExpression(col.alt);
