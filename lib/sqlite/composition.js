...
function resolveAncestorKeyExpressions(levels, rowRef, model) {
	...
	if (levels.length > 1) {
		...
		const childWhereClauses = [
			childKeys0.map((pk, j) => `${quote(pk)} = ${rowRef}.${quote(parentKeyBinding[j])}`).join(' AND ')
		];
...
function buildCompositionParentContext(compositionHierarchy, parentObjectIDs, rowRef, model, childObjectID = null, buildFieldObjectIDFn = null) {
	const { levels, isCompositionOfOne, compositionFieldChangelog, parentKeyBinding } = compositionHierarchy;
	const parentEntityName = levels[0].entityName;
	const compositionFieldName = levels[0].compositionFieldName;
