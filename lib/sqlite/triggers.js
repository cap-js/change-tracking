...
function generateUpdateTrigger(entity, columns, objectIDs, rootObjectIDs, model, compositionParentInfo = null, ancestorCompositionChain = []) {
	...
	if (compositionParentInfo) {
		compositionParentContext = buildCompositionParentContext(
			compositionParentInfo, rootObjectIDs, 'update', 'new', model,
			ancestorCompositionChain, ctx.objectID,
			(changelog, parentName, parentEntity, keyBinding) =>
				buildCompositionFieldObjectID(changelog, parentEntity, keyBinding, 'new', model)
		);
...
function generateDeleteTriggerPreserve(entity, columns, objectIDs, rootObjectIDs, model, compositionParentInfo = null, ancestorCompositionChain = []) {
	const ctx = buildTriggerContext(entity, objectIDs, 'old', model);

	let compositionParentContext = null;
	if (compositionParentInfo) {
		compositionParentContext = buildCompositionParentContext(
			compositionParentInfo, rootObjectIDs, 'delete', 'old', model,
			ancestorCompositionChain, ctx.objectID,
			(changelog, parentName, parentEntity, keyBinding) =>
