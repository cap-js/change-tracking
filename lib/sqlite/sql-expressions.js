...
function getWhereCondition(col, modification) {
	if (modification === 'update') {
		if (col.target && col.foreignKeys?.length) {
			return col.foreignKeys.map((fk) => { const q = quote(`${col.name}_${fk}`); return `old.${q} IS NOT new.${q}`; }).join(' OR ');
		}
		if (col.target && col.on?.length) {
			return col.on.map((m) => { const q = quote(m.foreignKeyField); return `old.${q} IS NOT new.${q}`; }).join(' OR ');
		}
...
function buildObjectIDSelect(objectIDs, entityName, entityKeys, refRow, model) {
	...
	// Build a CASE WHEN all fields are NULL THEN entityKey ELSE concat END
