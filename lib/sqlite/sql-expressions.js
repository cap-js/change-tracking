...
function getLabelExpr(col, refRow, entityKey, model, entityName = null) {
	...
	if (col.altExpressions && col.altExpressions.length > 0 && entityName) {
		...
		const sqlParts = col.altExpressions.map((xpr) =>
			buildExpressionSQL(xpr, entityName, refRow, model, SQLiteService.CQN2SQL, toSQL)
		);
...
function buildObjectIDSelect(objectIDs, entityName, entityKeys, refRow, model) {
	...
	for (const objectID of objectIDs) {
		...
		if (objectID.expression) {
			...
			objectID.selectSQL = toSQL(
				SELECT.one.from(entityName).columns(exprColumn).where(buildKeyWhere(entityKeys, refRow)),
				model
			);
		} else {
