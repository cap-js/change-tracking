...
function generateH2Trigger(csn, entity, rootEntity, mergedAnnotations = null) {
    model = csn;
    const trackedColumns = utils.extractTrackedColumns(entity, csn, mergedAnnotations);
    if (trackedColumns.length === 0) return null;
...
function _generateDeleteBody(entity) {
    const keys = utils.extractKeys(entity.keys);
    const whereClause = keys.map(k => `${k} = ?`).join(' AND ');
    const deleteSQL = `DELETE FROM sap_changelog_Changes WHERE ENTITY = ? AND ENTITYKEY = ?`;

    const keyExpr = keys.map(k => `oldRow.getString("${k}")`).join(' + "||" + ');

    return `
        String entityName = "${entity.name}";
        String entityKey = ${keyExpr};
        ${_wrapInTryCatch(deleteSQL, ['entityName', 'entityKey'])}
    `;
}
...
function _generateKeyCalculationJava(entity, rootEntity, ref, rootObjectIDs) {
    ...
    const rootObjectIDBlock = _generateRootObjectIDCalculation(rootObjectIDs, rootEntity, ref);
...
function _generateRootObjectIDCalculation(rootObjectIDs, rootEntity, refRow) {
