...
function generateH2Trigger(csn, entity, rootEntity, mergedAnnotations = null, rootMergedAnnotations = null) {
	...
	if (trackedColumns.length > 0) {
		...
		if (!config?.disableDeleteTracking) {
			deleteBody = config?.preserveDeletes ? _generateDeleteBodyPreserve(entity, trackedColumns, objectIDs, rootEntity, rootObjectIDs) : _generateDeleteBody(entity, trackedColumns, objectIDs, rootEntity, rootObjectIDs);
		}
...
function _generateCreateBody(entity, columns, objectIDs, rootEntity, rootObjectIDs) {
	...
	const columnBlocks = columns
		.map((col) => {
			...
			const allBindings = [...bindings, ...labelRes.bindings, 'entityName', 'entityKey', 'objectID', 'rootEntityName', 'rootEntityKey', 'rootObjectID'];
...
function _generateUpdateBody(entity, columns, objectIDs, rootEntity, rootObjectIDs) {
	...
	const columnBlocks = columns
		.map((col) => {
			...
			const allBindings = [...oldRes.bindings, ...newRes.bindings, ...oldLabelRes.bindings, ...newLabelRes.bindings, 'entityName', 'entityKey', 'objectID', 'rootEntityName', 'rootEntityKey', 'rootObjectID'];
...
function _generateCompOfManyTrigger(targetEntity, rootEntity, compInfo, rootObjectIDs) {
	...
	const createBody = !config?.disableCreateTracking ? _generateCompOfManyCreateBody(targetEntity, rootEntity, compInfo, binding, rootObjectIDs) : '';
	const updateBody = !config?.disableUpdateTracking ? _generateCompOfManyUpdateBody(targetEntity, rootEntity, compInfo, binding, rootObjectIDs) : '';
	const deleteBody = !config?.disableDeleteTracking ? _generateCompOfManyDeleteBody(targetEntity, rootEntity, compInfo, binding, rootObjectIDs) : '';
...
function _generateCompOfManyKeyCalcJava(targetEntity, rootEntity, compInfo, binding, rootObjectIDs, refRow) {
	...
	const parentEntityKeyExp = binding.map((k) => `${refRow}.getString("${k}")`).join(' + "||" + ');

	// objectID = value from target row based on alt paths (the child being added/removed)
	const objectIDExp = compInfo.alt && compInfo.alt.length > 0 ? compInfo.alt.map((p) => `${refRow}.getString("${p}")`).join(' + ", " + ') : 'null';

	// rootObjectID = lookup from root entity (used as objectID since entity is now the parent)
	const rootObjectIDBlock = _generateCompOfManyRootObjectIDCalc(rootEntity, rootObjectIDs, binding, refRow);

	// Log on the parent entity since 'compInfo.name' (e.g., 'children') is an attribute of the parent
	return `
        String entityName = "${rootEntity.name}";
        String parentEntityKey = ${parentEntityKeyExp};
        String childObjectID = ${objectIDExp};
        ${rootObjectIDBlock}
    `;
}
...
function _generateCompOfManyCreateBody(targetEntity, rootEntity, compInfo, binding, rootObjectIDs) {
	...
	// Log on the parent entity - rootEntity/rootEntityKey/rootObjectID are not needed since entity IS the root
	const insertSQL = `INSERT INTO sap_changelog_Changes 
	...
	const tryBlock = _wrapInTryCatch(insertSQL, ['childObjectID', 'entityName', 'parentEntityKey', 'rootObjectID']);
...
function _generateCompOfManyUpdateBody(targetEntity, rootEntity, compInfo, binding, rootObjectIDs) {
	...
	// Get old objectID
	const oldObjectIDExp = compInfo.alt && compInfo.alt.length > 0 ? compInfo.alt.map((p) => `oldRow.getString("${p}")`).join(' + ", " + ') : 'null';
	...
	const tryBlock = _wrapInTryCatch(insertSQL, ['oldChildObjectID', 'childObjectID', 'entityName', 'parentEntityKey', 'rootObjectID']);

	return `${keysCalc}
        String oldChildObjectID = ${oldObjectIDExp};
        ${tryBlock}`;
}
