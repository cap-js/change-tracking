...
// Session context variable name for skipping change tracking
const CT_SKIP_VAR = 'CT_SKIP_VAR';

function _getSkipCheckCondition() {
    return `(current_setting('${CT_SKIP_VAR}', true) IS NULL OR current_setting('${CT_SKIP_VAR}', true) != 'true')`;
}
...
function _generateFunctionBody(entity, columns, objectIDs, rootEntity, rootObjectIDs) {
    ...
    return `
        DECLARE
            rec RECORD;
        BEGIN
            -- Check if change tracking should be skipped for this service
            IF NOT ${_getSkipCheckCondition()} THEN
                RETURN NULL;
            END IF;
...
function _generateColumnSubquery(col, modification) {
    ...
    return `SELECT '${col.name}' AS attribute, ${oldValExp} AS valueChangedFrom, ${newValExp} AS valueChangedTo, '${col.type}' AS valueDataType WHERE ${whereCondition}`;
}
...
function _generateInsertBlock(columns, modification, entity) {
    ...
    if (modification === 'delete' && !config?.preserveDeletes) {
        ...
        return `DELETE FROM sap_changelog_changes WHERE entity = '${entity.name}' AND entitykey = ${entityKey};`;
    }

    const unionMembers = columns.map(col => _generateColumnSubquery(col, modification));
...
function _prepareVal(col, recordVar) {
    ...
    if ((col.type === 'cds.Association' || col.type === 'cds.Composition') && col.on) {
