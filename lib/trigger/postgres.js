...
function buildCompositionOfOneParentBlock(compositionParentInfo, rootObjectIDs, modification) {
	...
	const parentKeyExpr = `CONCAT_WS('||', ${parentKeys.map((pk) => 
		`(SELECT ${pk}::TEXT FROM ${utils.transformName(parentEntityName)} WHERE ${parentWhereClause})`
	).join(', ')})`;
	...
	if (rootObjectIDs?.length > 0) {
		const oidSelects = rootObjectIDs.map((oid) => 
			`(SELECT ${oid.name}::TEXT FROM ${utils.transformName(parentEntityName)} WHERE ${parentWhereClause})`
		);
		rootObjectIDExpr = oidSelects.length > 1 
			? `CONCAT_WS(', ', ${oidSelects.join(', ')})`
			: oidSelects[0];
	} else {
...
function buildCompositionParentBlock(compositionParentInfo, rootObjectIDs, modification, grandParentCompositionInfo = null) {
	...
	if (parentKeyBinding.type === 'compositionOfOne') {
		return buildCompositionOfOneParentBlock(compositionParentInfo, rootObjectIDs, modification);
	}
