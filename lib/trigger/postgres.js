...
const { CT_SKIP_VAR, getEntitySkipVarName } = require('../../cds-plugin.js');
...
function _getSkipCheckCondition(entityName) {
    ...
    return `(
        (current_setting('${CT_SKIP_VAR}', true) IS NULL OR current_setting('${CT_SKIP_VAR}', true) != 'true')
        AND
        (current_setting('${entitySkipVar}', true) IS NULL OR current_setting('${entitySkipVar}', true) != 'true')
    )`;
}
