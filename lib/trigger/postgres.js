...
function _generateColumnSubquery(col, modification, entity) {
    ...
    const oldLabelExp = modification === 'create' ? 'NULL' : _prepareLabelVal(col, 'OLD');
    const newLabelExp = modification === 'delete' ? 'NULL' : _prepareLabelVal(col, 'NEW');
    ...
    return `SELECT '${col.name}' AS attribute, ${oldValExp} AS valueChangedFrom, ${newValExp} AS valueChangedTo, ${oldLabelExp} AS valueChangedFromLabel, ${newLabelExp} AS valueChangedToLabel, '${col.type}' AS valueDataType WHERE ${fullWhereCondition}`;
}
...
// Returns the label expression for a column (looked-up value for associations with alt, NULL otherwise)
function _prepareLabelVal(col, recordVar) {
...
function handleAssocLookup(column, refRow) {
    ...
    const localizedInfo = utils.getLocalizedLookupInfo(column.target, column.alt, _model);
    
    if (localizedInfo) {
        // Build locale-aware lookup: try .texts table first, fall back to base entity
        const textsWhere = { ...where, locale: { val: `current_setting('$user.locale', true)` } };
        ...
        let textsSQL = _toPostgres(textsQuery);
        let baseSQL = _toPostgres(baseQuery);
        
        textsSQL = textsSQL.replace(/'(rec\.\w+)'/g, '$1').replace(/'((?:OLD|NEW)\.\w+)'/g, '$1');
        baseSQL = baseSQL.replace(/'(rec\.\w+)'/g, '$1').replace(/'((?:OLD|NEW)\.\w+)'/g, '$1');
