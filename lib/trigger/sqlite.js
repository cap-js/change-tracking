...
/**
 * Generates CREATE TRIGGER IF NOT EXISTS for INSERT events
 */
...
function generateCompOfManyCreateTrigger(targetEntity, rootEntity, compInfo, rootObjectIDs) {
	...
	return `CREATE TRIGGER IF NOT EXISTS ${triggerName} AFTER INSERT
    ON ${targetTableName}
...
function generateCompOfManyUpdateTrigger(targetEntity, rootEntity, compInfo, rootObjectIDs) {
	...
	return `CREATE TRIGGER IF NOT EXISTS ${triggerName} AFTER UPDATE
    ON ${targetTableName}
    WHEN ${getSkipCheckCondition(targetEntity.name)}
    BEGIN
        ${insertSQL}
...
function generateCompOfManyDeleteTrigger(targetEntity, rootEntity, compInfo, rootObjectIDs) {
	...
	return `CREATE TRIGGER IF NOT EXISTS ${triggerName} AFTER DELETE
    ON ${targetTableName}
    WHEN ${getSkipCheckCondition(targetEntity.name)}
    BEGIN
        ${insertSQL}
...
function generateSQLiteTrigger(csn, entity, rootEntity, mergedAnnotations = null, rootMergedAnnotations = null) {
	model = csn;
	const triggers = [];
	const { columns: trackedColumns, compositionsOfMany } = utils.extractTrackedColumns(entity, model, mergedAnnotations);
