...
function generateSQLiteTriggers(entity, rootEntity, mergedAnnotations = null) {
	const triggers = [];
...
function _generateColumnSubquery(col, modification, entity, entityKey) {
	...
	return `SELECT '${col.name}' AS attribute, ${valueFrom} AS valueChangedFrom, ${valueTo} AS valueChangedTo, '${col.type}' AS valueDataType ${fromClause} WHERE ${fullWhereCondition}`;
}
