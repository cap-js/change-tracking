...
function _getSkipCheckCondition() {
	return `(session_context('${CT_SKIP_VAR}') IS NULL OR session_context('${CT_SKIP_VAR}') != 'true')`;
}
...
function _generateUpdateTrigger(entity, columns, objectIDs, rootEntity, rootObjectIDs) {
	...
	const entriesSQL = columns
        ...
        WHERE ${whereCondition} AND ${_getSkipCheckCondition()};`;
		})
		.join('\n');
...
function _generateDeleteTriggerPreserve(entity, columns, objectIDs, rootEntity, rootObjectIDs) {
	...
	const entriesSQL = columns
        ...
        WHERE ${_buildWhereClauseCondition(col, keys, 'old')} AND ${_getSkipCheckCondition()};`;
		})
		.join('\n');
...
function _generateDeleteTrigger(entity) {
    ...
    WHEN ${_getSkipCheckCondition()}
    BEGIN
        DELETE FROM ${utils.transformName('sap.changelog.Changes')} WHERE entity = '${entity.name}' AND entityKey = ${entityKey} AND ${_getSkipCheckCondition()};
    END;`;
}
