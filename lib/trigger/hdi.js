...
/**
 * Builds SQL expression for looking up the parent changelog ID
 */
function buildParentLookupExpr(compositionParentInfo, rowRef) {
	if (!compositionParentInfo) return 'NULL';
	...
	// Look up the parent changelog entry that was created for this composition in the same transaction
...
function generateCreateTrigger(entity, columns, objectIDs, rootEntity, rootObjectIDs, compositionParentInfo = null) {
	...
	// If this is a composition child with tracked columns, build context for composition parent entry
	...
	const body = wrapInSkipCheck(entity.name, insertSQL, compositionParentContext);
...
function generateDeleteTrigger(entity, columns, objectIDs, rootEntity, rootObjectIDs, compositionParentInfo = null) {
	...
	// For delete triggers with composition, order is: DELETE old history -> check/INSERT composition parent -> INSERT child entries
	const compositionParentContext = compositionParentInfo
		? buildCompositionParentContext(compositionParentInfo, rootObjectIDs, 'delete', 'old')
		: null;
...
function generateHANATriggers(csn, entity, rootEntity = null, mergedAnnotations = null, rootMergedAnnotations = null) {
	...
	for (const comp of compositionsOfMany) {
		...
		// Check if target entity has its own tracked columns
		// If yes, the target's own _CT_* triggers will handle the composition entry (merged triggers)
		const { columns: targetTrackedColumns } = utils.extractTrackedColumns(targetEntity, model, null);
		if (targetTrackedColumns.length > 0) {
			// Skip generating separate composition triggers - they'll be merged into target's triggers
			continue;
		}

		// Generate composition triggers only if target has NO tracked columns
		if (!config?.disableCreateTracking) {
