...
function _generateUpdateTrigger(entity, columns, objectIDs, rootEntity = null, rootObjectIDs = null) {
	...
	const body = columns.map((col) => {
		...
		const checkCols = col.foreignKeys ? col.foreignKeys.flatMap(fk => `${col.name}_${fk}`) : col.structFields ? [...col.structFields.map(f => col.name + '_' + f.name)] : [col.name];
	...
	const ofColumns = columns.flatMap(c => {
		if (c.structFields) {
			return c.structFields.map(f => `${c.name}_${f.name}`);
		}
...
function _prepareValue(col, refRow) {
	...
	if (col.target && col.alt) {
		...
		} else if (col.structFields) {
			raw = col.structFields?.map(f => `COALESCE(TO_NVARCHAR(:${refRow}.${col.name}_${f.name}), '')`).join(" || ' ' || ");
		}
		valExp = raw;
	}

	if (col.structFields) {
		condition = col.structFields?.map(f => `:${refRow}.${col.name}_${f.name} IS NOT NULL`).join(' OR ');
