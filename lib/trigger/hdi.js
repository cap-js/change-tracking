	...
	if (modification === 'update') {
		...
		const checkCols = col.foreignKeys
			...
			: col.on
				...
				: [col.name];
		// Pass type for scalar columns (non-associations) to handle LOB types
		const colType = (!col.foreignKeys && !col.on) ? col.type : null;
		return checkCols.map(k => _nullSafeChanged(k, 'old', 'new', colType)).join(' OR ');
	} else {
		...
		} else if (col.type === 'cds.LargeBinary' || col.type === 'cds.Binary') {
			// SAP HANA cannot directly compare LOB types with IS NOT NULL in triggers
			return `LENGTH(:${rowRef}.${col.name}) > 0`;
		} else {
...
function _nullSafeChanged(column, oldRef = 'old', newRef = 'new', type = null) {
	const o = `:${oldRef}.${column}`;
	const n = `:${newRef}.${column}`;
	// Special handling for LOB types
	if (type === 'cds.LargeBinary' || type === 'cds.Binary') {
		return `LENGTH(${n}) != LENGTH(${o})`;
	}
...
function _generateDeleteTrigger(entity, columns, objectIDs, rootEntity = null, rootObjectIDs = null) {
	...
	const body = `IF ${_getSkipCheckCondition(entity.name)} THEN
		...
		-- Then insert delete changelog entries
		INSERT INTO SAP_CHANGELOG_CHANGES 
		(ID, attribute, valueChangedFrom, valueChangedTo, entity, entityKey, objectID, rootEntity, rootEntityKey, rootObjectID, createdAt, createdBy, valueDataType, modification, transactionID)
		SELECT
			SYSUUID,
...
function _generateCompOfManyCreateTrigger(targetEntity, rootEntity, compInfo, rootObjectIDs) {
	...
	const entityKey = targetKeys.map(k => `TO_NVARCHAR(:new.${k})`).join(" || '||' || ");
	const rootEntityKey = binding.map(k => `TO_NVARCHAR(:new.${k})`).join(" || '||' || ");
...
function _generateCompOfManyDeleteTrigger(targetEntity, rootEntity, compInfo, rootObjectIDs) {
	...
	const targetKeys = utils.extractKeys(targetEntity.keys);
	const entityKey = targetKeys.map(k => `TO_NVARCHAR(:old.${k})`).join(" || '||' || ");
	const rootEntityKey = binding.map(k => `TO_NVARCHAR(:old.${k})`).join(" || '||' || ");
