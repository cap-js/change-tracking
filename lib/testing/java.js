...
const buildDatabaseProxy = async (cds, from) => {
  ...
  const isDbView = (entityName) => {
    ...
    for (let i = 1; i < parts.length; i++) if (linked.definitions[parts.slice(0, i).join('.')]?.kind === 'service') return false;
  ...
  if (hcqlDatabaseProxy.definitions[changeViewProxyKeyForDb]) {
    ...
    for (const dbName of changeTrackedDbNames) {
      ...
      proxyEntity.elements.changes = {
        ...
        on: [{ ref: ['changes', 'entityKey'] }, '=', keyExpr.length === 1 ? keyExpr[0] : { xpr: keyExpr }, 'and', { ref: ['changes', 'entity'] }, '=', { val: dbName }]
      };
    }
  }
...
async function java(...args) {
  ...
  const unlinkLockSync = () => {
    try {
      ...
      /* lock file already gone or unreadable — nothing to clean up */
    }
  };
  ...
  const app = await new Promise((resolve, reject) => {
    ...
    const javaCmd = _buildJavaLaunchCommand(jarFile, p);
...
/**
 * Build the Java launch command. Since @cap-js/change-tracking switched from
 * H2's inline `AS $$...$$` trigger form (which invokes the system javac at
 * DB init and therefore needs `org.h2.tools.TriggerAdapter` on the system
 * classpath) to the `CALL '<fqcn>'` form (which references a class compiled
 * at Maven build time and packaged in the Spring Boot exec jar), the plain
 * `java -jar app.jar` launch works again.
 */
function _buildJavaLaunchCommand(jarFile, p) {
  return { cmd: 'java', args: ['-jar', jarFile, `--server.port=${p}`] };
}
