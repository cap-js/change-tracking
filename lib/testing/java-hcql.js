  ...
  async init() {
    ...
    this.on('*', async (req) => {
      ...
      const originalTarget =
        req.query.INSERT?.into?.ref?.[0] ??
        req.query.UPSERT?.into?.ref?.[0] ??
        req.query.UPDATE?.entity?.ref?.[0] ??
        null;
      const originalDef =
        (typeof originalTarget === 'string' && cds.model.definitions[originalTarget]) || null;
