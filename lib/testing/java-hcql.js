...
class JavaHcqlService extends cds.Service {
  async init() {
    ...
    this.on('*', async (req) => {
      ...
      //
      // Escape ALL regex metacharacters (not just `.`) when interpolating `from`
      // into a RegExp. CDS entity names shouldn't contain any of these today, but
      // the fully-generic escape silences CodeQL's `js/incomplete-sanitization`
      // and future-proofs the code against exotic identifiers.
      for (const [from, to] of Object.entries(proxyMap)) {
        const escaped = from.replace(/[\\^$.*+?()[\]{}|]/g, '\\$&');
        json = json.replace(new RegExp(`(?<=\\[|"id":)"${escaped}"`, 'g'), `"${to}"`);
      }
