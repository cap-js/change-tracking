...
async function generatePathAndParams (req, entityKey) {
  ...
  if (hasParentAndForeignKey && (parentEntity[isRoot] || cds.model.definitions[parentEntity.name]?.[isRoot])) {
    return compContext;
  }

  let parentAssoc = await processEntity(targetEntity, targetKey, compContext);
  let isParentRoot = parentAssoc.entity[isRoot] || cds.model.definitions[parentAssoc.entity.name][isRoot];
  while (parentAssoc && !isParentRoot) {
...
async function track_changes (req) {
  ...
  let m = cds.model.definitions[target.name];
  let isRootEntity = target[isRoot] || m[isRoot];
  const params = convertSubjectToParams(req.subject);
  if (req.subject.ref.length === 1 && params.length === 1 && !isRootEntity) {
