...
function convertSubjectToParams(subject) {
  ...
  return subjectRef.length>=0 ? subjectRef : params;
}
...
async function pathAndParams(entity, entityKey) {
  const serviceInfo = {
    path: '/' + entity.name,
    params:[
      {
  ...
  let parentAssociation = await processTest(entity, entityKey, serviceInfo);
  while (parentAssociation && parentAssociation.entity['change-tracking-isRootEntity'] === false) {
    serviceInfo.path = '/' + parentAssociation.entity.name + serviceInfo.path
    parentAssociation = await processTest(parentAssociation.entity, parentAssociation.ID, serviceInfo);
  }
  serviceInfo.path = parentAssociation.entity.name + serviceInfo.path
  return serviceInfo;
}

async function processTest(target, entityKey, serviceInfo) {
  if (target['change-tracking-isRootEntity'] === false) {
    const targetInfo = target['change-tracking-parentEntity'].associationName;
    const parentEntity = target.elements[targetInfo]._target;
    let resParentOne = await SELECT.one.from(target.name).where({ID: entityKey}).columns(targetInfo);
    serviceInfo.params.unshift({ID: resParentOne[`${targetInfo}_ID`]});
    return {
      entity: parentEntity,
      ID: resParentOne[`${targetInfo}_ID`]
    };
  } else return;
}
...
async function track_changes (req) {
  ...
  // const testSubject = await pathAndParams(target, entityKey);
  // let isComposition = _isCompositionContextPath(testSubject.path)
  let isComposition = _isCompositionContextPath(req.path)
  // if (target['change-tracking-isRootEntity'] === false) {
  //   const resParentOne = await SELECT.one.from(target.name).where({ID: entityKey}).columns(target.associationName)
  //   // const key = resParentOne.ID;
  //   serviceInfo.unshift({ID: resParentOne.ID});
  // }
  ...
  // const resParent1 = await SELECT.one.from(target.name).columns('orderItem_ID')
  // req.event = "DELETE"
  if (
    req.event === "DELETE" &&
    target["change-tracking-isRootEntity"] === true &&
  ...
  if (isComposition) {
    ...
    [ target, entityKey ] = await _prepareChangeLogForComposition(target, entityKey, changes, {data:req.data,context:{path: req.path,params, event: req.event}})
    // [ target, entityKey ] = await _prepareChangeLogForComposition(target, entityKey, changes, {data:req.data,context:{path: testSubject.path,params:testSubject.params, event: req.event}})
  }
