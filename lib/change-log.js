...
const _getRootEntityPathVals = function (txContext, entity, entityKey) {
  ...
  if (txContext.event === "CREATE") {
    ...
    const curEntity = getEntityByContextPath(path, txContext.hasCompContext)
  ...
  while (_isCompositionContextPath(path, txContext.hasCompContext)) {
    const hostEntity = getEntityByContextPath(path = path.slice(0, -1), txContext.hasCompContext)
...
const _getAllPathVals = function (txContext) {
  ...
  for (let idx = 0; idx < paths.length; idx++) {
    const entity = getEntityByContextPath(paths.slice(0, idx + 1), txContext.hasCompContext)
...
const _isCompositionContextPath = function (aPath, hasCompContext) {
  if (!aPath) return
  if (typeof aPath === 'string') aPath = aPath.split('/')
  if (aPath.length < 2) return false
  const target = getEntityByContextPath(aPath, hasCompContext)
  const parent = getEntityByContextPath(aPath.slice(0, -1), hasCompContext)
...
async function generatePathAndParams(req, entityKey) {
  ...
  let compContext = {
    ...
    params: hasParentAndForeignKey ? [{ [ID]: entityKey }, { [ID]: data[foreignKey] }] : [{ [ID]: entityKey }]
  };
...
async function track_changes (req) {
  ...
  let isComposition = compContext?.path ? true : _isCompositionContextPath(req.path, compContext?.hasCompContext)
  if (
    req.event === "DELETE" &&
    target[isRoot] &&
