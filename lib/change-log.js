...
function getAssociationDetails (entity) {
  if (!entity) return {};
  const assocName = entity[hasParent]?.associationName || cds.model.definitions[entity.name]?.[hasParent]?.associationName;
...
async function track_changes (req) {
  ...
  if (req.subject.ref.length === 1 && params.length === 1 && (!target[isRoot] || !m[isRoot])) {
