...
const _formatPropertyContext = async function (changes, reqData) {
  for (const change of changes) {
    ...
    const a = cds.model.definitions[change.serviceEntity].elements[semkeys[0].split(".")[0]]
    ...
    const condition = a.on.reduce((conditions, e, i) => {
      if (e === "=") {
        ...
        if(sourceProperty !== change.attribute) {
          throw new Error("Unmanaged associations need to only use conditions based on the changed property")
        }
